---
path: "src/widgets/custom/remote-video/remotevideo.mdx"
...
<blockquote>
  **Multi-platform video widget** that automatically fetches metadata from YouTube, Vimeo, Twitch,
  and TikTok, storing rich video information for easy embedding.
</blockquote>
...
- ✅ **Multi-Platform**: YouTube, Vimeo, Twitch, TikTok support
- ✅ **Automatic Metadata**: Fetches title, thumbnail, duration, channel info
- ✅ **Server-Side APIs**: Protects API keys, prevents CORS issues
- ✅ **Rich Preview**: Visual thumbnail and video details display
- ✅ **Structured Data**: Stores comprehensive video object, not just URL
- ✅ **Debounced Input**: Performance-optimized with 500ms delay
- ✅ **Loading States**: Visual feedback during metadata fetching
- ✅ **Error Handling**: Clear error messages for invalid URLs or API failures
- ✅ **Platform Validation**: Valibot-based URL and platform validation
- ✅ **Unified Error Handler**: Uses `handleWidgetValidation` for consistent error management
- ✅ **Unified Error Handler**: Uses `handleWidgetValidation` for consistent error management
- ✅ **Non-Translatable**: Video metadata is universal
- ✅ **Tailwind CSS**: Fully styled using Tailwind utility classes (no custom CSS)

---

## When to Use RemoteVideo Widget

### ✅ Use RemoteVideo When:
...
The RemoteVideo widget requires a server endpoint to fetch metadata. Create `/api/http/remoteVideo` route:

```typescript
// src/routes/api/http/remoteVideo/+server.ts
...
| `required`         | `boolean`  | ❌ No     | `false`                                    | Whether video is mandatory                 |
| `placeholder`      | `string`   | ❌ No     | `'Enter video URL...'`                     | Input placeholder text                     |
| `allowedPlatforms` | `string[]` | ❌ No     | `['youtube', 'vimeo', 'twitch', 'tiktok']` | Restrict to specific platforms             |
| `helper`           | `string`   | ❌ No     | `undefined`                                | Helper text                                |
| `translated`       | `boolean`  | 🔒 Locked | `false`                                    | Always false (video metadata is universal) |
...
2. **Don't Embed Too Many Videos**
   ...
   // ✅ Use thumbnails, load iframe on click
   ```
