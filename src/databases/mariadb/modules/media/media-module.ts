...
export class MediaModule {
	...
	public readonly files = {
		...
		restore: async (fileId: DatabaseId, _tenantId?: string | null): Promise<DatabaseResult<void>> => this.adapter.crud.restore('media', fileId),

		delete: (fileId: DatabaseId, _tenantId?: string | null): Promise<DatabaseResult<void>> => this.adapter.crud.delete('media', fileId),
		...
		getMetadata: (fileIds: DatabaseId[], tenantId?: string | null): Promise<DatabaseResult<Record<string, MediaMetadata>>> =>
			this.adapter.wrap(async () => {
				const res = await this.adapter.crud.findMany<MediaItem>('media', { _id: { $in: fileIds } } as any, { tenantId });
		...
		duplicate: async (fileId: DatabaseId, newName?: string, tenantId?: string | null): Promise<DatabaseResult<MediaItem>> => {
			const res = await this.adapter.crud.findOne<MediaItem>('media', { _id: fileId } as any, { tenantId });
			if (!res.success || !res.data) return res as any;
			const newItem = { ...res.data, _id: undefined, filename: newName || `copy_${res.data.filename}`, createdAt: undefined, updatedAt: undefined };
	...
	public readonly folders = {
		...
		delete: (folderId: DatabaseId, _tenantId?: string | null): Promise<DatabaseResult<void>> => this.adapter.crud.delete('media_folders', folderId),

		deleteMany: (folderIds: DatabaseId[], tenantId?: string | null): Promise<DatabaseResult<{ deletedCount: number }>> =>
			this.adapter.crud.deleteMany('media_folders', { _id: { $in: folderIds } } as any, { tenantId }),
		...
		getFolderContents: (
			...
			this.adapter.wrap(async () => {
				const [foldersRes, filesRes] = await Promise.all([
					this.adapter.crud.findMany<MediaFolder>('media_folders', { parentId: folderId || null } as any, { tenantId }),
		...
		move: (folderId: DatabaseId, targetParentId?: DatabaseId, tenantId?: string | null): Promise<DatabaseResult<MediaFolder>> =>
			this.adapter.crud.update('media_folders', folderId, { parentId: targetParentId || null } as any, tenantId)
	};
