/**
 * @file src/routes/api/http/systemPreferences/+server.ts
 * @description Consolidated server-side API endpoint for managing system and user preferences.
 *
 * ### Usage
 * - GET /api/http/systemPreferences?key=... - Loads a specific preference value for the authenticated user.
 * - GET /api/http/systemPreferences?keys[]=...&keys[]=... - Loads multiple preference values for the user.
 * - POST /api/http/systemPreferences with `{ key, value }` - Saves a single preference for the user.
 * - POST /api/http/systemPreferences with `[{ key, value }, ...]` - Saves multiple preferences in a single request.
 * - DELETE /api/http/systemPreferences?key=... - Deletes a specific preference for the user.
 *
 * ### Features
