/**
 * @file src\routes\api\dashboard\system-info\+server.ts
 ...
 *
 * Usage:
 * GET /api/http/dashboard/systemInfo - Full system information
 * GET /api/http/dashboard/systemInfo?type=cpu - Only CPU information
 * GET /api/http/dashboard/systemInfo?type=disk - Only disk information
 * GET /api/http/dashboard/systemInfo?type=memory - Only memory information
 * GET /api/http/dashboard/systemInfo?type=network - Only network information
 * GET /api/http/dashboard/systemInfo?type=os - Only OS information
 * GET /api/http/dashboard/systemInfo?type=process - Only process information
 *
...
const fetchDiskInfo = async () => {
  ...
  if (
    rootDiskUsageResult.success &&
    "data" in rootDiskUsageResult &&
    Array.isArray(rootDiskUsageResult.data)
  ) {
    // v2.x format - find the main filesystem
    const diskData =
      rootDiskUsageResult.data.find((d: { filesystem: string }) =>
        d.filesystem.includes("/dev/"),
      ) ||
  ...
  try {
    ...
    logger.warn(
      "Failed to get detailed mount points, falling back to root:",
      error instanceof Error ? error.message : String(error),
    );
    ...
    if (os.platform() !== "win32") {
      ...
      if (swapLine) {
        ...
        swapInfo = {
          ...
          usedSwapPercentage: Number.parseFloat(
            ((Number.parseInt(parts[2], 10) / Number.parseInt(parts[1], 10)) * 100).toFixed(2),
          ),
          freeSwapPercentage: Number.parseFloat(
            ((Number.parseInt(parts[3], 10) / Number.parseInt(parts[1], 10)) * 100).toFixed(2),
          ),
        };
      }
    } else {
      ...
      if (lines.length > 1) {
        ...
        swapInfo = {
          ...
          freeSwapPercentage: Number.parseFloat(
            (((totalSwap - usedSwap) / totalSwap) * 100).toFixed(2),
          ),
        };
      }
    }
    ...
    if (os.platform() === "win32") {
      ...
      if (lines.length > 1) {
        const processData = lines.slice(1).map((line) => {
          ...
          const totalTime =
            (Number.parseInt(parts[4] || "0", 10) + Number.parseInt(parts[3] || "0", 10)) /
            10_000_000; // Convert 100ns to seconds
          return {
      ...
      // Linux/Unix - use ps
      const { stdout } = await execAsync(
        `ps -eo pid,comm,%cpu,%mem,rss --sort=-%cpu | head -n ${PROCESS_LIMIT + 1}`,
      );
      const lines = stdout.trim().split("\n");
...
const getSystemInfo = async (type?: string) => {
  try {
    ...
    if (type === "network") {
      return {
        networkInfo: await getCachedOrFresh("network", fetchNetworkInfo),
      };
    }
    if (type === "process") {
      return {
        processInfo: await getCachedOrFresh("process", fetchProcessInfo),
      };
    }
