/**
 * @file src/routes/api/http/media/remote/+server.ts
 * @description
 * API endpoint for saving remote media files to the current tenant's storage.
 *
 * @example POST /api/http/media/remote
 *
 * Features:
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const validAccess: MediaAccess = ["public", "private", "protected"].includes(access)
      ? access
      : "public";
