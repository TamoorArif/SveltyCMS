...
SveltyCMS includes a global hotkey system built into `src/utils/hotkeys.ts` to improve navigation and task efficiency, especially for power users and those relying on keyboard navigation.

### Guidelines for Hotkeys
- **Discoverability**: When implementing a hotkey for a specific action (like Save or Delete), ensure the corresponding button indicates the shortcut via the `aria-keyshortcuts` attribute or in a tooltip.
- **Cross-Platform**: The `Mod` modifier automatically maps to `Ctrl` on Windows/Linux and `Cmd` (⌘) on macOS.
- **Conflict Prevention**: Global hotkeys must not interfere with native browser commands or typing in form fields. The hotkey system automatically ignores events originating from `INPUT`, `TEXTAREA`, `SELECT`, and `contenteditable` elements.

#### Common Standard Shortcuts
Please adhere to the following standard patterns when adding new hotkeys to SveltyCMS:

| Action | Shortcut(s) | Platforms | Notes / Rationale |
| :--- | :--- | :--- | :--- |
| **Save** | `Mod + S` | All | Universal save (browser, macOS, Windows apps, Notion, VS Code, etc.) |
| **Cancel / Close** | `Escape` | All | Standard for modals, dialogs, search close |
| **Delete** | `Delete` / `Backspace` | Windows/Linux | Forward delete on Win; Backspace often deletes too |
| **Delete (Mac)** | `Mod + Backspace` | macOS | Mac has no dedicated forward Delete on laptops; ⌘+Backspace = trash/delete |
| **Publish / Submit** | `Mod + Enter` | All | Common "force confirm" (Notion publish, many editors) |
| **Global Search** | `Mod + K` | All | Modern apps (Linear, Notion cmd/ctrl+K, VS Code, many admin panels) |
| **Undo** | `Mod + Z` | All | Native browser + app standard |
| **Redo** | `Mod + Shift + Z` | All | Standard pairing with undo |
| **Help / Cheatsheet** | `?` or `Mod + /` | All | Discoverability |
