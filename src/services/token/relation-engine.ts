...
import { canAccessCollection } from "./relation-resolver";
...
export async function getRelationTokens(
  ...
  const relationFields = (schema.fields as FieldInstance[]).filter(
    (field) => field.widget?.Name === "Relation",
  );
  ...
  for (const field of relationFields) {
    ...
    if (!hasAccess) {
      logger.debug(
        `User ${user?._id} denied access to relation ${fieldName} → ${relatedCollection}`,
      );
      continue;
    }
