...
export async function cleanupExpiredDemoTenants() {
  ...
  const isDemoEnv =
    ...
    (globalThis as any).process?.env?.SVELTYCMS_DEMO === "true";
  ...
  try {
    ...
    const tenantIds = [
      ...new Set(expiredTenantUsers.map((u) => u.tenantId).filter(Boolean)),
    ] as string[];
    ...
    const allTenantUsers = usersResult.data.filter(
      (u) => u.tenantId && tenantIds.includes(u.tenantId),
    );
    ...
    for (const tenantId of tenantIds) {
      ...
      try {
        await db.crud.deleteMany("content_nodes", { tenantId } as Record<string, unknown>);
        await db.crud.deleteMany("content_drafts", { tenantId } as Record<string, unknown>);
      ...
      try {
        ...
        const tenantThemes = (allThemes || []).filter(
          (t) => (t as unknown as Tenanted).tenantId === tenantId,
        );
