/**
 * @file src/routes/api/http/remoteVideo/+server.ts
...
export const POST = apiHandler(async ({ request }) => {
  try {
    ...
    if (!videoData) {
      const errorMsg =
        "Could not fetch video metadata. The URL may be invalid or the platform is unsupported.";
    ...
    if (
      ...
      !allowedPlatforms.includes(videoData.platform)
    ) {
    ...
    logger.info(
      `API /remoteVideo: Successfully fetched metadata for ${videoData.platform} video.`,
      { url },
    );
