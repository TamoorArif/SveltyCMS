...
```http
GET /api/dashboard/last5-content
```
...
```http
GET /api/dashboard/last5-content?tenantId=tenant-abc
```
...
// Get online users
const usersResponse = await fetch("/api/dashboard/online-user", {
  credentials: "include",
});
...
// Get recent content
const contentResponse = await fetch("/api/dashboard/last5-content", {
  credentials: "include",
});
