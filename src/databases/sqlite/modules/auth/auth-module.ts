...
export class AuthModule implements IAuthAdapter {
	...
	async createUser(userData: Partial<User>): Promise<DatabaseResult<User>> {
		return this.core.wrap(async () => {
			...
			logger.debug(`[SQLite/Auth] Creating user: ${values.email}, isAdmin: ${values.isAdmin}, roles: ${JSON.stringify(values.roleIds)}`);
	...
	async updateUserAttributes(userId: string, userData: Partial<User>, _tenantId?: string | null): Promise<DatabaseResult<User>> {
		return this.core.wrap(async () => {
			const now = isoDateStringToDate(nowISODateString());
	...
	async deleteUsers(userIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ deletedCount: number }>> {
		return this.core.wrap(async () => {
	...
	async blockUsers(userIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ modifiedCount: number }>> {
		return this.core.wrap(async () => {
			await this.db.update(schema.authUsers).set({ blocked: true }).where(inArray(schema.authUsers._id, userIds));
			return { modifiedCount: userIds.length };
		}, 'BLOCK_USERS_FAILED');
	}

	async unblockUsers(userIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ modifiedCount: number }>> {
		return this.core.wrap(async () => {
			await this.db.update(schema.authUsers).set({ blocked: false }).where(inArray(schema.authUsers._id, userIds));
			return { modifiedCount: userIds.length };
		}, 'UNBLOCK_USERS_FAILED');
	}

	async getUserById(userId: string, _tenantId?: string | null): Promise<DatabaseResult<User | null>> {
		return this.core.wrap(async () => {
			const [result] = await this.db
				.select()
				.from(schema.authUsers)
	...
	async getUserByEmail(criteria: { email: string; tenantId?: string | null }): Promise<DatabaseResult<User | null>> {
		return this.core.wrap(async () => {
	...
	// Session methods
	async createSession(sessionData: { user_id: string; expires: ISODateString; tenantId?: string | null }): Promise<DatabaseResult<Session>> {
		return this.core.wrap(async () => {
	...
	async validateSession(sessionId: string): Promise<DatabaseResult<User | null>> {
		return this.core.wrap(async () => {
			const [session] = await this.db.select().from(schema.authSessions).where(eq(schema.authSessions._id, sessionId)).limit(1);
			...
			const [user] = await this.db.select().from(schema.authUsers).where(eq(schema.authUsers._id, session.user_id)).limit(1);
	...
	async updateSessionExpiry(sessionId: string, expires: ISODateString): Promise<DatabaseResult<Session>> {
		return this.core.wrap(async () => {
			await this.db
			...
			const [result] = await this.db.select().from(schema.authSessions).where(eq(schema.authSessions._id, sessionId)).limit(1);
			return utils.convertDatesToISO(result) as unknown as Session;
		}, 'UPDATE_SESSION_EXPIRY_FAILED');
	}

	async invalidateAllUserSessions(userId: string, _tenantId?: string | null): Promise<DatabaseResult<void>> {
		return this.core.wrap(async () => {
			await this.db.delete(schema.authSessions).where(eq(schema.authSessions.user_id, userId));
		}, 'INVALIDATE_USER_SESSIONS_FAILED');
	}

	async getActiveSessions(userId: string, _tenantId?: string | null): Promise<DatabaseResult<Session[]>> {
		return this.core.wrap(async () => {
			const results = await this.db.select().from(schema.authSessions).where(eq(schema.authSessions.user_id, userId));
	...
	async getTokenByValue(token: string, _tenantId?: string | null): Promise<DatabaseResult<Token | null>> {
		return this.core.wrap(async () => {
			const [result] = await this.db.select().from(schema.authTokens).where(eq(schema.authTokens.token, token)).limit(1);
			return result ? (utils.convertDatesToISO(result) as unknown as Token) : null;
		}, 'GET_TOKEN_FAILED');
	}

	async getTokenData(token: string, _userId?: string, _type?: string, tenantId?: string | null): Promise<DatabaseResult<Token | null>> {
	...
	async deleteTokens(tokenIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ deletedCount: number }>> {
		return this.core.wrap(async () => {
	...
	async blockTokens(tokenIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ modifiedCount: number }>> {
		return this.core.wrap(async () => {
			await this.db.update(schema.authTokens).set({ blocked: true }).where(inArray(schema.authTokens._id, tokenIds));
			return { modifiedCount: tokenIds.length };
		}, 'BLOCK_TOKENS_FAILED');
	}

	async unblockTokens(tokenIds: string[], _tenantId?: string | null): Promise<DatabaseResult<{ modifiedCount: number }>> {
		return this.core.wrap(async () => {
			await this.db.update(schema.authTokens).set({ blocked: false }).where(inArray(schema.authTokens._id, tokenIds));
			return { modifiedCount: tokenIds.length };
		}, 'UNBLOCK_TOKENS_FAILED');
	}

	async updateToken(tokenId: string, tokenData: Partial<Token>, _tenantId?: string | null): Promise<DatabaseResult<Token>> {
		return this.core.wrap(async () => {
			await this.db
				.update(schema.authTokens)
				.set(tokenData as any)
				.where(eq(schema.authTokens._id, tokenId));
			const [res] = await this.db.select().from(schema.authTokens).where(eq(schema.authTokens._id, tokenId)).limit(1);
	...
	async getRoleById(roleId: string, _tenantId?: string | null): Promise<DatabaseResult<Role | null>> {
		return this.core.wrap(async () => {
			const [result] = await this.db
				.select()
				.from(schema.roles)
	...
	async updateRole(roleId: string, roleData: Partial<Role>, _tenantId?: string | null): Promise<DatabaseResult<Role>> {
		return this.core.wrap(async () => {
			const now = isoDateStringToDate(nowISODateString());
			const updateData = {
				...roleData,
