...
import { GET as getFlow, PATCH as updateFlow, DELETE as deleteFlow } from '@src/routes/api/automations/[id]/+server';
...
describe('Automation API Security - IDOR and Tenant Isolation', () => {
	...
	describe('IDOR Protection for specific Automations ([id])', () => {
		...
		it('should allow getting if the flow belongs to the current tenant', async () => {
			(automationService.getFlow as any).mockResolvedValue({ id: flowId, tenantId: myTenant } as any);
