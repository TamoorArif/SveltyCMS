...
const functions: Record<string, ModifierFunction> = {
  ...
  min: (v, args) =>
    String(Math.min(Number.parseFloat(String(v)), Number.parseFloat(args?.[0] || "0"))),
  max: (v, args) =>
    String(Math.max(Number.parseFloat(String(v)), Number.parseFloat(args?.[0] || "0"))),
  number: (v, args) => {
    const decimals = Number.parseInt(args?.[0] || "0", 10);
  ...
  gt: (v, args) =>
    Number.parseFloat(String(v)) > Number.parseFloat(args?.[0] || "0") ? "true" : "false",
  lt: (v, args) =>
    Number.parseFloat(String(v)) < Number.parseFloat(args?.[0] || "0") ? "true" : "false",
