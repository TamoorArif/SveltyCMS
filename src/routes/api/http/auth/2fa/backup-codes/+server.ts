/**
 * @file src/routes/api/http/auth/2fa/backup-codes/+server.ts
 * @description
...
export const POST = apiHandler(async ({ locals }) => {
  ...
  return json({
    ...
    message:
      "New backup codes generated. Please save these codes in a secure location. Your old backup codes are no longer valid.",
  });
});
