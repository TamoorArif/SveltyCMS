	...
	async loadMetrics(): Promise<Record<string, ServicePerformanceMetrics>> {
		...
		try {
			...
			if (snapshotResult.success && snapshotResult.data) {
				const snapshot = typeof snapshotResult.data === 'string' ? JSON.parse(snapshotResult.data) : snapshotResult.data;
			...
			const legacyResult = await dbAdapter.system.preferences.getByCategory('performance', 'system');
