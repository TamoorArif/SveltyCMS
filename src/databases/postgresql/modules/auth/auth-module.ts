...
export class AuthModule {
	...
	private mapUser(dbUser: typeof schema.authUsers.$inferSelect): User {
		...
		return {
			...
			permissions: (converted as { permissions?: string[] }).permissions || []
		} as User;
	}
	...
	async getUserCount(filter?: Record<string, unknown>): Promise<DatabaseResult<number>> {
		return this.core.wrap(async () => {
			...
			const where = filter ? (this.core.mapQuery(table as unknown as Record<string, unknown>, filter) as import('drizzle-orm').SQL) : undefined;
