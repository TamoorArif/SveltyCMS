/**
 * @file src/routes/api/http/media/external/[...path]/+server.ts
 ...
 *
 * ## POST /api/http/media/external/process
 *
 * ### Request Body
 * ```json
 * {
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    logger.info(
      `Received external processing request for mediaId: ${mediaId} via service: ${service}`,
      {
        parameters,
      },
    );
