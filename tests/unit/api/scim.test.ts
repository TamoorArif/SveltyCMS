...
vi.mock('@utils/scim-utils', () => ({
	...
	scimError: vi.fn((status: number, detail: string) => new Response(JSON.stringify({ detail, status }), { status })),
	buildScimUser: vi.fn((user: any) => ({ id: user?._id || 'unknown', userName: user?.email || 'unknown' })),
	buildScimGroup: vi.fn((role: any) => ({ id: role?._id || 'unknown', displayName: role?.name || 'unknown' })),
...
describe('SCIM API Unit Tests', () => {
	...
	describe('SCIM Users', () => {
		...
		it('POST /Users should create user in tenant', async () => {
			...
			expect(mockAuth.checkUser).toHaveBeenCalledWith({ email: 'new@t1.com', tenantId: 'tenant-1' });
			expect(mockAuth.createUser).toHaveBeenCalledWith(expect.objectContaining({ email: 'new@t1.com', tenantId: 'tenant-1' }));
		});
	});
	...
	describe('SCIM Groups', () => {
		...
		it('DELETE /Users/[id] should deactivate user', async () => {
			...
			expect(mockAuth.updateUser).toHaveBeenCalledWith('u1', expect.objectContaining({ blocked: true }), 'tenant-1');
		});
		...
		it('PATCH /Groups/[id] should update membership in tenant via bulk update', async () => {
			...
			mockAuth.authInterface.getRoleById.mockResolvedValue({ success: true, data: { _id: 'r1', name: 'Role 1' } });
