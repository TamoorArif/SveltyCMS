...
import { fallback as dispatcher } from "./http/[...path]/+server";
...
async function runBenchmark() {
    ...
    for (let i = 0; i < iterations; i++) {
        const mockEvent = {
            request: new Request(`http://localhost/api/http/collections/${targetCollection}`),
...
async function benchmarkAuth(dbAdapter: any, dispatcher: any) {
    ...
    const sdkStart = performance.now();
    for (let i = 0; i < iterations; i++) {
        try { await cms.auth.login(credentials); } catch(e) {}
    }
    const sdkEnd = performance.now();
    const sdkAvg = (sdkEnd - sdkStart) / iterations;
    ...
    for (let i = 0; i < iterations; i++) {
        const mockEvent = {
            request: new Request("http://localhost/api/http/auth/login", { 
                method: "POST", 
