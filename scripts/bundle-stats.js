...
function checkRegressions(currentStats, history) {
  ...
  if (currentBuildTime > 0) {
    ...
    if (percentChange > 50) {
      ...
      console.log(
        `${COLORS.red}Please investigate recent changes causing this slowdown.${COLORS.reset}`,
      );
...
function generateSuggestions(largeFiles, totalSize) {
  ...
  if (largeFiles.length > 0) {
    ...
    suggestions.push(
      `  - Use ${COLORS.cyan}dynamic imports (import())${COLORS.reset} to code-split heavy components/routes.`,
    );
    suggestions.push("  - Check for large dependencies that can be lazy-loaded.");
    suggestions.push(
      `  - Run ${COLORS.cyan}npm run build:analyze${COLORS.reset} to visualize the bundle structure.`,
    );
  }
  ...
  if (poorlyCompressed.length > 0) {
    ...
    suggestions.push(
      "  - Verify if these files are already compressed binaries (images, etc) included in the bundle.",
    );
  }
...
function printHistory(history) {
  ...
  recent.forEach((entry) => {
    ...
    console.log(
      `  ${date.padEnd(25)} ${formatBytes(currentSize).padEnd(10)} ${printComparison(currentSize, prevSize)}`,
    );
  });
  console.log("");
}
...
function generateReport(results, metadata = {}) {
  ...
  console.log(
    `  Total Size:    ${formatBytes(stats.totalSize)}  ${prevBuild ? printComparison(stats.totalSize, prevBuild.totalSize) : ""}`,
  );
  console.log(
    `  Gzip Size:     ${formatBytes(stats.totalGzip)}  ${prevBuild ? printComparison(stats.totalGzip, prevBuild.totalGzip) : ""}`,
  );
  console.log(
    `  Brotli Size:   ${formatBytes(stats.totalBrotli)}  ${COLORS.green}(Real-world transfer size)${COLORS.reset}`,
  );
  ...
  if (largeFiles.some((f) => f.size > CONFIG.budgets.maxChunkSize)) {
    console.error(
      `${COLORS.red}❌ Build failed: Individual chunk size limit exceeded.${COLORS.reset}`,
    );
    process.exit(1);
  }
