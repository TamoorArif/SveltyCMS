/** @file src/routes/api/http/metrics/+server.ts @description Unified metrics API endpoint for monitoring and observability features: [Prometheus-compatible export, JSON metrics, real-time performance data, admin-only reset] */

// Services
import { metricsService } from "@src/services/metrics-service";
import { json } from "@sveltejs/kit";
/**
 * GET /api/http/metrics
...
export const POST = apiHandler(async ({ locals, request }) => {
  ...
  try {
    ...
    throw new AppError(
      'Invalid action. Use {"action": "reset"} to reset metrics.',
      400,
      "INVALID_ACTION",
    );
  } catch (err) {
