/**
 * @file src/routes/api/http/dashboard/online-user/+server.ts
...
export const GET = apiHandler(async ({ locals }) => {
  ...
  if (!auth) {
    ...
    throw new AppError(
      "Internal Server Error: Auth service unavailable.",
      500,
  ...
  if (!sessionsResult.success) {
    ...
    throw new AppError(
      `Failed to retrieve active sessions: ${sessionsResult.message || "Unknown error"}`,
      500,
      "SESSION_FETCH_ERROR",
    );
  }
