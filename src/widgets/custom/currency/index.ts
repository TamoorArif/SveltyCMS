...
const validationSchema = (field: FieldInstance) => {
  ...
  if (field.minValue !== undefined) {
    validationActions.push(
      minValue(field.minValue as number, `Value must be at least ${field.minValue}.`),
    );
  }
  if (field.maxValue !== undefined) {
    validationActions.push(
      maxValue(field.maxValue as number, `Value must not exceed ${field.maxValue}.`),
    );
  }
  ...
  const schema =
    validationActions.length > 0 ? pipe(baseSchema, ...(validationActions as any)) : baseSchema;
