...
graph TD
    ...
    subgraph Database ["Database (State Persistence)"]
...
> [!NOTE]
> **Categories (Folders)** are strictly structural metadata nodes (`nodeType: 'category'`). Unlike collections, categories do not correspond to files on disk. They exist exclusively within the database to organize the collections hierarchically.
...
| **Orphaned Database Nodes**   | The `ContentManager` validation logic detects nodes with invalid structure and prevents them from loading into memory.         |
