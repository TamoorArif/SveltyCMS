...
The Media API provides a robust and type-safe system for managing media files (images, videos, documents) in SveltyCMS. All operations are handled by the **MediaService** and **MediaProcessingService**, which use a database-agnostic adapter and enforce **strict multi-tenant isolation** and **advanced security hardening** (SSRF prevention, command injection protection).
...
| POST   | `/api/media/transcode`           | Transcode video assets     |
...
**Note on Filenames:** SveltyCMS automatically sanitizes filenames during upload to ensure filesystem compatibility. This includes removing special characters and normalizing date prefixes (e.g., `2024-01-01` becomes `20240101`). The `url` and `path` properties in the response reflect this sanitized filename, which may differ slightly from the original `filename`.
...
> [!IMPORTANT]
> **SSRF Protection**: Remote URLs are strictly validated. Only HTTPS is allowed, and all internal/private IP ranges (RFC 1918, loopback, link-local) are blocked to prevent Server-Side Request Forgery attacks.
...
> [!NOTE]
> **Security**: The Transform API is hardened against directory traversal. All paths are resolved and validated against the `MEDIA_ROOT`.
...
### 15. Transcode Video Asset

Transcode a video file to another format (e.g., HLS for adaptive streaming).
...
| Field          | Type   | Description                                   |
| -------------- | ------ | --------------------------------------------- |
| `mediaId`      | string | ID of the media item to transcode             |
| `targetFormat` | string | Target format: `hls` (default), `mp4`, `webm` |

**Permissions Required:** `media:update` or admin.
...
```json
{
  ...
  "data": {
    "status": "processing",
    "outputDir": "/mediaFolder/transcoded/vid_123"
  }
}
```

> [!TIP]
> **Security**: Transcoding is handled via secure process spawning (no shell execution) to prevent command injection.
...
- [**Media Gallery Guide**](../guides/media-gallery.mdx) - DAM features and asset organization
- [**Image Editor Guide**](../guides/image-editor.mdx) - Technical details on the canvas-based editor
- [**AI Integration**](../guides/development/ai-integration.mdx) - Details on the MCP server and RAG architecture
...
15. ✅ `POST /api/media/transcode` - Video transcoding (HLS/Adaptive)
...
7. **Security Hardened** - Native SSRF protection, command injection prevention (spawn-based), and strict tenant isolation
8. **Storage Analytics** - AI-powered insights and consumption predictions
9. **No Plugin Required** - All features built-in, not dependent on third-party plugins
...
>
> - SveltyCMS: Sharp.js on Node.js 24, local SSD storage
