...
import { exportSettings, encryptSensitiveData, createExportMetadata } from '@src/utils/server/export-utils';
...
export const POST = apiHandler(async ({ request, locals, url }) => {
	...
	switch (type) {
		...
		default:
			throw new AppError('Invalid export type or not yet supported in this endpoint', 400, 'INVALID_EXPORT_TYPE');
	}
