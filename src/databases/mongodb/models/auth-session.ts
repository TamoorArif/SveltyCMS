...
export class SessionAdapter {
  ...
  constructor() {
    ...
    this.SessionModel =
      (mongoose.models?.auth_sessions as Model<Session>) ||
      ...
      if (options.invalidateOthers !== false) {
        ...
        const filter: Record<string, unknown> = {
          ...
          $or: [
            ...
            {
              rotated: true,
              expires: { $lte: new Date(now.getTime() + 60_000).toISOString() },
  ...
  async rotateToken(oldToken: string, expires: ISODateString): Promise<DatabaseResult<string>> {
    try {
      ...
      logger.info(
        `Token rotated successfully - old: ${oldToken} (grace period until ${graceExpiry.toISOString()}), new: ${newSession._id}`,
      );
      return {
        success: true,
        data: newSession._id,
  ...
  async updateSessionExpiry(sessionId: string, newExpiry: Date): Promise<DatabaseResult<Session>> {
    try {
      ...
      return {
        success: true,
        data: this.formatSession(
          session as unknown as Parameters<SessionAdapter["formatSession"]>[0],
        ),
      };
    } catch (err) {
    ...
    try {
      ...
      const results = await this.SessionModel.aggregate([
        ...
        // Stage 7: Consolidate user data with role info and session metadata
        {
          $addFields: {
            ...
            "user.permissions": {
              $setUnion: [
                { $ifNull: ["$user.permissions", []] },
                { $ifNull: ["$roleInfo.permissions", []] },
              ],
            },
      ...
      if (results.length > 0) {
        ...
        if (user._sessionRotated && user._sessionRotatedTo) {
          logger.debug(
            `Session ${sessionId} was rotated to ${user._sessionRotatedTo}, but still valid during grace period`,
          );
        }
    ...
    try {
      ...
      return {
        ...
        data: sessions.map((session) =>
          this.formatSession(session as unknown as Parameters<SessionAdapter["formatSession"]>[0]),
        ),
      };
    } catch (err) {
  ...
  async getAllActiveSessions(tenantId?: string | null): Promise<DatabaseResult<Session[]>> {
    try {
      ...
      logger.debug("All active sessions retrieved", {
        count: sessions.length,
        tenantId,
      });
      return {
