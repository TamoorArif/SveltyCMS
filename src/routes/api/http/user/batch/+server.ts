/**
 * @file src/routes/api/http/user/batch/+server.ts
...
export const POST: RequestHandler = apiHandler(async ({ request, locals }) => {
  ...
  if (userIds.some((id) => id === user?._id)) {
    throw new AppError(
      "You cannot perform batch actions on your own account.",
      400,
      "SELF_ACTION_DENIED",
    );
  }
  ...
  if (getPrivateSettingSync("MULTI_TENANT")) {
    ...
    if (userChecks.some((u: any) => u === null)) {
      ...
      throw new AppError(
        "Forbidden: One or more user IDs do not belong to your tenant or do not exist.",
        403,
        "FORBIDDEN_TENANT",
      );
    }
    ...
    case "delete": {
      ...
      for (const userId of userIds) {
        ...
        if (result.success && result.data) {
          ...
          const errorMsg =
            !result.success && "error" in result ? result.error?.message : "Unknown error";
