...
sequenceDiagram
    ...
    Note over UI, API: Dashboard Initialization (Batch)
    UI->>API: GET /api/settings/all
    API->>ServerCache: Fetch All Authorized Groups
    ServerCache-->>API: Merged Settings Object
    API-->>UI: Optimized JSON Response

    Note over User, DB: Individual Group Update
    User->>UI: Changes a setting (e.g., SMTP_HOST)
    UI->>API: PUT /api/settings/[group]
    API->>DB: Updates database record
    DB-->>API: Success
    API->>ServerCache: Invalidate & Reload Cache
    API->>API: Increment Settings Version
    API-->>UI: 200 OK

    loop Every 5 seconds (Public Sync)
        ClientStore->>API: GET /api/settings/public/version
        API-->>ClientStore: New Version Detected
        ClientStore->>API: GET /api/settings/public
        API-->>ClientStore: Refreshed Public Settings
        ClientStore->>UI: Global UI elements update
    end
```

### Server-Side

1.  **Batch Aggregation:** The `/api/settings/all` endpoint aggregates settings from all authorized groups into a single payload, reducing HTTP overhead during dashboard initialization.
2.  **Server Startup:** The application loads all settings from the database into a fast, in-memory cache.
3.  **Admin Update:** An admin uses the UI to change a setting. The UI sends a `PUT` request to the `/api/settings/[group]` endpoint.
4.  **Database Update:** The API updates the setting in the database.
5.  **Cache Invalidation:** The API calls `invalidateSettingsCache()` and `loadSettingsFromDB()` to immediately refresh the server-side cache with the new value.
6.  **Version Update:** The API calls `updateVersion()` to bump the settings version number. This signals to clients that settings have changed.
...
<Callout type="info">
  **Note:** AI features require an active connection to the configured provider. If using Ollama,
  ensure the server is running and the specified vision model is pulled (`ollama pull llava`).
</Callout>
...
<Callout type="error">
  **CRITICAL: Data Loss Risk When Removing Content Languages** Removing a language from
  `AVAILABLE_CONTENT_LANGUAGES` after content has been created can result in **permanent data loss**
  for translations in that language.
</Callout>
