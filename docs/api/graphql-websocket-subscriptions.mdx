...
- ✅ **JIT-Optimized Resolvers**: Subscriptions leverage the high-performance JIT execution engine.
- ✅ Multitenant support
- ✅ Permission-based subscription filtering
- ✅ Automatic reconnection handling
- ✅ GraphQL introspection support
- 🛡️ **Zero-Dependency AST Security**: Depth limiting (8 levels) applied natively to subscription streams

---

## Real-time Performance

SveltyCMS subscriptions utilize the **Just-In-Time (JIT)** execution engine via `@envelop/graphql-jit`. This ensures that even high-frequency event streams are processed with minimal latency and CPU overhead.

- **Pre-compiled Resolvers**: Subscription resolvers are compiled into optimized machine-ready code.
- **Efficient Filtering**: Real-time permission checks and tenant filtering are executed at native speed.
- **Low Latency**: Sub-millisecond processing time for event broadcasting to connected clients.
