/**
 * @file src/routes/api/http/collections/[collectionId]/batch/+server.ts
 * @description API endpoint for batch operations on collection entries
 *
 * @example: POST /api/http/collections/posts/batch
 *
 * Features:
...
export const POST = apiHandler(async ({ locals, params, request }) => {
  ...
  if (action === "delete") {
    ...
    const res = await dbAdapter.crud.updateMany(
      normalizedName,
      query,
