...
if (typeof setInterval !== "undefined") {
  setInterval(
    () => {
      ...
      logger.trace(
        `Session cache cleanup: ${strongRefs.size} strong refs, ${sessionCache.size} weak refs`,
      );
    },
    5 * 60 * 1000,
  );
...
async function getUserFromSession(
  ...
  try {
    ...
    logger.error(
      `Session validation failed for ${sessionId.substring(0, 8)}: ${err instanceof Error ? err.message : String(err)}`,
    );
  }
...
async function handleSessionRotation(
  ...
  try {
    ...
    if (newSession && newSession._id !== oldSessionId) {
      ...
      const cacheKey = event.locals.tenantId
      ...
      logger.info(
        `Session rotated for user ${user._id}: ${oldSessionId.substring(0, 8)}... → ${newSessionId.substring(0, 8)}...`,
      );
    }
  } catch (err) {
    // Non-fatal error - log but don't break the session
    logger.error(
      `Session rotation failed for ${oldSessionId.substring(0, 8)}: ${err instanceof Error ? err.message : String(err)}`,
    );
...
export const handleAuthentication: Handle = async ({ event, resolve }) => {
  ...
  try {
    ...
    if (sessionId) {
      ...
      if (!auth) {
        logger.debug(
          "Auth service not ready during session validation - skipping validation but preserving cookie",
        );
      ...
      if (user) {
        ...
        try {
          ...
          if (isTestMode || !(await rotationRateLimiter.isLimited(event))) {
            ...
            logger.trace(
              `Session rotation rate limited for session ${sessionId.substring(0, 8)}...`,
            );
          }
        } catch (rotationError) {
          ...
          if (
            ...
            !rotationError.message.includes("Session expired")
          ) {
