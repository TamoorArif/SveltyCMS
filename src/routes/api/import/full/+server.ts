...
import type { Conflict, DatabaseId, ExportData, ExportMetadata, ImportOptions, ImportResult, ValidationResult } from '@src/content/types';
 ...
 */
async function applyImport(importData: ExportData, options: ImportOptions, conflicts: Conflict[], tenantId: string): Promise<ImportResult> {
	...
	if (importData.settings) {
		const allFields = (await import('@src/routes/(app)/config/system-settings/settings-groups')).settingsGroups.flatMap((g) => g.fields);
				...
				try {
					...
					if (existing.success && existing.data) {
						if (options.strategy === 'overwrite') {
							await dbAdapter.crud.update(collectionName, doc._id as DatabaseId, { ...doc, tenantId }, tenantId as any);
							result.imported++;
						}
						// skip/merge logic...
					} else {
					...
					result.errors.push({ key: collection.name, message: String(e), code: 'COLLECTION_IMPORT_FAILED' });
				}
			}
		}
	}
...
async function logImport(user: any, metadata: ExportMetadata, result: ImportResult, tenantId: string, dryRun: boolean): Promise<void> {
	try {
		const { logAuditEvent, AuditEventType } = await import('@src/services/audit-log-service');
		await logAuditEvent({
			action: dryRun ? `Dry-run system import: ${metadata.export_id}` : `Full system import: ${metadata.export_id}`,
			actorId: user._id,
...
async function validateImportData(data: ExportData): Promise<ValidationResult> {
	...
	if (!data.metadata || !data.metadata.exported_at) {
		return { valid: false, errors: [{ path: 'metadata', message: 'Invalid metadata', code: 'INVALID' }], warnings: [] };
	}
...
export const POST = apiHandler(async ({ locals, request, url }) => {
	...
	if (getPrivateSettingSync('MULTI_TENANT')) {
		...
		if (targetTenantId !== tenantIdFromLocals && !isSuperAdmin) throw new AppError('Tenant mismatch', 403, 'FORBIDDEN');
	}
	...
	if (importData.hasSensitiveData && importData.encryptedSensitive && options.sensitivePassword) {
		const decrypted = await decryptSensitiveData(importData.encryptedSensitive, options.sensitivePassword);
		importData.settings = { ...importData.settings, ...decrypted };
	}

	const validation = await validateImportData(importData);
	if (!validation.valid) return json({ success: false, errors: validation.errors }, { status: 400 });

	const conflicts = await detectConflicts(importData, targetTenantId);

	if (options.dryRun) {
		const dryRunResult: ImportResult = { success: true, imported: 0, skipped: 0, merged: 0, errors: [], conflicts };
