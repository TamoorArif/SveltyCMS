...
export const POST = apiHandler(async ({ locals, request }) => {
  ...
  try {
    ...
    if (!hasWidgetPermission) {
      logger.warn(
        `User ${user._id} (role: ${user.role}) denied access to API /api/widgets due to insufficient role permissions`,
      );
    ...
    logger.debug("[/api/widgets/status] Clearing widget active cache and related collections", {
    ...
    try {
      ...
      logger.warn(
        "[/api/widgets/status] Enhanced cache clearing failed (non-critical):",
        cacheError,
      );
    }
