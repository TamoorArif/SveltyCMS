...
title: "GraphQL API"
description: "Complete GraphQL API reference for querying and mutating data in SveltyCMS with Vite+ optimized dynamic schema generation."
order: 11
icon: "mdi:graphql"
author: "admin"
created: "2025-10-05"
...
- 🛡️ **Zero-Dependency Security**: AST-level depth limiting and alias throttling

---

## Security & Threat Protection

SveltyCMS employs an enterprise-grade, native **Security Response Service** (`src/services/security-response-service.ts`) to prevent Denial of Service (DoS), payload anomalies, and injection attacks.

### Why a Custom "GraphQL Armor"?

Rather than relying on generic third-party middleware (like `@escape.tech/graphql-armor`), SveltyCMS implements a proprietary security layer specifically optimized for our high-speed Vite+ routing:

1. **Deep Payload Anomaly Detection**: The custom `SecurityResponseService` performs recursive JSON payload scanning _before_ queries hit the GraphQL engine. It actively detects SQLi, Command Injection, XSS, Path Traversal, and LDAP Injection.
2. **Unified Threat Perimeter**: Rate limits, IP blocking, and threat monitoring (via Redis) are shared across _all_ endpoints (REST, Auth, SCIM, GraphQL). A malicious payload in GraphQL instantly blocks that IP from hitting the authentication endpoints.
3. **Escalation State Machine**: The service groups threats by IP/Tenant and escalates incidents dynamically (Moderate, High, Critical) based on indicator frequency and severity, triggering automatic webhooks and cooldown periods.

### AST Validation Rules

The GraphQL endpoint strictly validates all incoming queries using custom Abstract Syntax Tree (AST) rules:

**1. Depth Limiting:**
Deeply nested queries (e.g., recursive relations) are blocked to prevent CPU and memory exhaustion. The maximum allowed query depth is **8 levels**. Queries exceeding this will be rejected before execution with a `GraphQLError`.

**2. Alias Throttling:**
To prevent malicious batching attacks where an attacker requests the same expensive field multiple times using different aliases, the API limits queries to a maximum of **15 aliases**.

### Schema Introspection

By default, schema introspection is **disabled in production** to prevent attackers from mapping the API structure. It is automatically enabled in development environments and when using the `dev` mode.

---

## Performance & Execution Engine

To meet enterprise throughput demands, the GraphQL implementation utilizes a **Just-In-Time (JIT) Execution Engine** via the `@envelop/graphql-jit` plugin.

- **JIT Compilation**: GraphQL queries are compiled down to optimized JavaScript functions on their first execution.
- **Microsecond Resolution**: Subsequent identical queries execute significantly faster by bypassing the standard GraphQL AST validation and resolution phases.
- **Memory Efficiency**: Reduces overhead for complex queries, keeping garbage collection (GC) pressure low under high concurrency.
- **Benchmark Proven**: Combined with our native routing, this achieves sub-millisecond API response overhead for hot paths.
...
When multi-tenant mode is enabled, SveltyCMS provides robust, mathematically proven schema and data isolation:

- **Per-Tenant Yoga Instances**: SveltyCMS dynamically instantiates and caches a separate GraphQL Yoga server for each tenant. This completely eliminates "Schema Leakage", ensuring that custom collections or fields defined by Tenant A are never visible in the introspection queries of Tenant B.
- **Data Isolation**: Queries are automatically scoped to the tenant context injected by the authentication middleware.
- **Cross-Tenant Prevention**: Users can only access their own tenant data. Cross-tenant queries are blocked at the database adapter level.
...
SveltyCMS uses the **Just-In-Time (JIT)** execution engine via `@envelop/graphql-jit`. This significantly improves performance by compiling GraphQL queries into highly optimized JavaScript functions before execution.

- 🚀 **Faster Execution**: Up to 10x faster than standard execution for repetitive queries.
- 📉 **Lower Memory Overhead**: Reduced garbage collection pressure during high-traffic periods.
- ⚡ **Optimized Resolvers**: Compiles resolver chains for maximum throughput.
...
const userResponse = await fetch("/api/user/create-user", {
  method: "POST",
