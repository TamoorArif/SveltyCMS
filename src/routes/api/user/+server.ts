...
export const GET = apiHandler(async ({ url, locals }) => {
	...
	const [usersResult, totalUsersResult] = await Promise.all([dbAdapter.auth.getAllUsers(options), dbAdapter.auth.getUserCount(filter)]);
	...
	if (!totalUsersResult.success) {
		throw new AppError(totalUsersResult.message || 'Failed to get user count', 500, 'DB_COUNT_ERROR');
	}
...
export const POST = apiHandler(async ({ request, locals, url }) => {
	if (!auth) {
		throw new AppError('Internal Server Error: Authentication system not initialized', 500, 'AUTH_SYS_ERROR');
	}
