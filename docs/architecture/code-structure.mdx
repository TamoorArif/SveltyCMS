---
path: "docs/architecture/code-structure.mdx"
...
See [Widget System Architecture](./widgets/architecture.mdx) for details.
...
- **`date-utils.ts`**: Standardized ISO-8601 boundary operations without heavy moment.js/date-fns overhead.
...
2. **URL Navigation**: Server returns `contentLanguage` → Syncs to app singleton
3. **User Toggle**: translation-status dropdown → Updates app singleton → Navigates with `invalidateAll: true`
...
```
User Action → Singleton Update → Server Request → Cache Lookup → Widget Render
                    ↓              ↓               ↓              ↓
...
- **[Widget System](../guides/development/widgets/index.mdx)** - Widget architecture
