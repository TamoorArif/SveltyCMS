...
2.  **Session Cookie**: `demo_tenant_id` (Demo Mode) or `svelty_tenant` (Standard).
3.  **API Header**: `X-Tenant-ID` (for API integration).

4.  **Registration**: New users can register without a token (if configured) to create a brand new isolated tenant.
