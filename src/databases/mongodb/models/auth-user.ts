...
export class UserAdapter {
  ...
  private mapUser(user: any): User {
    ...
    if (result.last2FAVerification) {
      result.last2FAVerification = toISOString(
        result.last2FAVerification as string | number | Date,
      );
    }
    if (result.createdAt) {
  ...
  async updateUserAttributes(
    ...
    try {
      const filter = safeQuery({ _id: userId } as any, tenantId, {
  ...
  async getAllUsers(
    ...
    try {
      const filter = safeQuery(options?.filter || {}, options?.filter?.tenantId as string, {
      ...
      const mappedUsers = users.map((user) =>
        this.mapUser(user as unknown as Record<string, unknown>),
      );
      return {
        success: true,
        data: mappedUsers,
  ...
  async getUserCount(
    ...
    try {
      const safeFilter = safeQuery(filter || {}, filter?.tenantId as string, {
        bypassTenantCheck: options?.bypassTenantCheck,
      });
  ...
  async getUserByEmail(
    ...
    try {
      ...
      const filter = safeQuery({ email: normalizedEmail } as any, criteria.tenantId, {
        bypassTenantCheck: options?.bypassTenantCheck,
      });
