...
export class MongoDBAdapter implements IDBAdapter {
	...
	media: any = { files: { restore: (id: any, t: any) => this.crud.restore('media', id, { tenantId: t }) } };
	system: any = {};
	monitoring: any = {};
	batch: any = {};
	collection: any = {};
	...
	async _createCrudAdapter(): Promise<ICrudAdapter> {
		...
		return {
			...
			insert: (c, d, t, s) => getRepo(c).insert(d as any, { tenantId: t as any, bypassTenantCheck: s }),
			insertMany: (c, d, t, s) => getRepo(c).insertMany(d as any[], { tenantId: t as any, bypassTenantCheck: s }),
			update: (c, id, d, t, s) => getRepo(c).update(id, d as any, { tenantId: t as any, bypassTenantCheck: s }),
			delete: (c, id, o) => getRepo(c).delete(id, o as any),
			findByIds: (c, ids, o) => getRepo(c).findByIds(ids, o as any),
			updateMany: (c, q, d, t, s) => getRepo(c).updateMany(q, d as any, { tenantId: t as any, bypassTenantCheck: s }),
			deleteMany: (c, q, o) => getRepo(c).deleteMany(q, o as any),
			upsert: (c, q, d, t, s) => getRepo(c).upsert(q, d as any, { tenantId: t as any, bypassTenantCheck: s }),
			upsertMany: (c, items, t, s) => getRepo(c).upsertMany(items, { tenantId: t as any, bypassTenantCheck: s }),
	...
	private _getOrCreateModel(name: string): mongoose.Model<any> {
		...
		const model = mongoose.model(name, new mongoose.Schema({}, { strict: false, timestamps: true }));
