/**
 * @file src/routes/api/http/metrics/unified/+server.ts
 ...
 *
 * @endpoint GET /api/http/metrics/unified
 */

import { metricsService } from "@src/services/metrics-service";
import { json } from "@sveltejs/kit";
/**
 * GET /api/http/metrics/unified
  ...
  try {
    ...
    const enhancedMetrics = {
      ...
      api: {
        ...
        apiHealthy:
          metricsReport.api.requests > 0
            ? metricsReport.api.errors / metricsReport.api.requests < 0.05
            : true,
        cacheEffective: metricsReport.api.cacheHitRate > 70,

        // API performance classification
        errorRateStatus: classifyErrorRate(
          metricsReport.api.requests > 0
            ? (metricsReport.api.errors / metricsReport.api.requests) * 100
            : 0,
        ),
      },
      ...
      security: {
        ...
        securityHealthy:
          metricsReport.security.rateLimitViolations < 20 &&
          metricsReport.security.cspViolations < 10 &&
          metricsReport.security.authFailures < 10,
      },
...
function calculatePerformanceScore(metrics: ReturnType<typeof metricsService.getReport>): number {
  const factors = {
    ...
    security: Math.max(
      0,
      10 -
        (metrics.security.rateLimitViolations / 5 +
          metrics.security.cspViolations / 2 +
          metrics.security.authFailures / 3),
    ),
  };
...
function calculateThreatLevel(security: {
  ...
  const violationScore =
    security.rateLimitViolations + security.cspViolations * 2 + security.authFailures * 1.5;
