...
export const contentCache = {
  ...
  async populateCache(tenantId?: string | null): Promise<void> {
    try {
      ...
      await cacheService.set("cms:content_structure", state, ttl, tenantId, CacheCategory.CONTENT, [
        "cms:content",
      ]);
      logger.debug(
        `[ContentCache] Populated content structure cache for tenant: ${tenantId || "global"}`,
      );
    } catch (error) {
   ...
   */
  async warmEntriesCache(
    collectionId: string,
    entryIds: string[],
    tenantId?: string | null,
  ): Promise<void> {
    const cacheService = await getCacheService();
    logger.info(`[ContentCache] Warming cache for ${entryIds.length} entries in ${collectionId}`, {
      tenantId,
    });
