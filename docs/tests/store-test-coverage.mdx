...
### 2. Loading Store (`loading-store.test.ts`)

**Tests:** 22 | **Status:** ⚠️ Requires browser environment

The loading-store uses Svelte 5 runes which require a browser runtime. These tests validate the **design and API contract** but need a browser test environment to execute.
...
### 3. Screen Size Store (`screen-size.test.ts`)
...
### 4. Setup Store (`setup-store.test.ts`)
...
bun test tests/bun/stores/loading-store.test.ts
bun test tests/bun/stores/screen-size.test.ts
bun test tests/bun/stores/setup-store.test.ts
```
...
- [Architecture: System State](../architecture/state-management.mdx)
