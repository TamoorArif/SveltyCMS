...
These actions are defined in `src/routes/setup/+page.server.ts`, but heavily delegate driver execution and database logic to the tree-shakeable `setup-service.server.ts` service.
...
| `createIfMissing` | boolean | Automatically create the database if possible |

**Config Object:**

```typescript
{
  type: 'mongodb' | 'mariadb' | 'postgresql' | 'sqlite';
  host: string;
  port: number;
  name: string;
  user?: string;
...
| Field    | Type   | Description                                          |
| -------- | ------ | ---------------------------------------------------- |
| `dbType` | string | `'mongodb'`, `'mariadb'`, `'postgresql'`, `'sqlite'` |
