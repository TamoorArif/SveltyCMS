...
addSecurityHeaders, // 1. MUST be first → headers on ALL responses
handleTestIsolation, // 2. Test context & worker isolation
handleStaticAssetCaching, // 3. Highest hit-rate early exit
handleCompression, // 4. Streaming-safe compression
handleSystemState, // 5. Readiness gate & auto-init
...
SveltyCMS middleware is optimized for **world-class micro-latency**, utilizing fast-path short-circuiting for static assets and API requests. The entire pipeline overhead is effectively "free" in modern production environments. For a full breakdown of system metrics, see the [Central Performance Benchmarks](../project/benchmarks.mdx).
