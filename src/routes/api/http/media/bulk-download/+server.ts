/**
 * @file src/routes/api/http/media/bulk-download/+server.ts
 * @description API endpoint for bulk downloading multiple media files as TAR.GZ archive
 *
 * @example POST /api/http/media/bulk-download
 *
 * Features:
...
import {
  cleanupArchive,
  createBulkDownloadArchive,
  streamArchiveToResponse,
} from "@utils/media/bulk-download";
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const archive = await createBulkDownloadArchive(
      files as unknown as import("@utils/media/media-models").MediaBase[],
      outputDir,
    );
