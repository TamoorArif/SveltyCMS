...
export async function getSetupDatabaseAdapter(
	...
	if (process.env.TEST_MODE === 'true' && config.host === 'mock-host') {
		...
		const mockAdapter = {
			...
			crud: { count: async () => 0 },
			getConnectionHealth: async () => ({ success: true, data: { healthy: true, latency: 10, activeConnections: 1 } })
...
async function setupMongoDB(config: DatabaseConfig, connectionString: string, correlationId: string): Promise<IDBAdapter> {
	...
	try {
		...
		try {
			...
			logger.warn(`⚠️ Auth probe warning: ${probeErr.message} (code: ${probeErr.code})`, { correlationId });
...
async function setupMariaDB(_config: DatabaseConfig, connectionString: string, _correlationId: string): Promise<IDBAdapter> {
...
async function setupPostgreSQL(_config: DatabaseConfig, connectionString: string, _correlationId: string): Promise<IDBAdapter> {
