/**
 * @file src/routes/api/http/importer/external/+server.ts
...
export const POST = apiHandler(async ({ request, locals }) => {
  ...
  if (!finalMapping) {
    ...
    if (!targetCol) {
      throw new AppError(
        `Target collection ${targetCollection} not found or access denied`,
        404,
  ...
  for (const item of externalData.items) {
    try {
      ...
      const result = await dbAdapter.crud.insert(
