/**
 * @file src/routes/api/http/media/trash/+server.ts
 * @description
 * API endpoint for moving a media file to the trash within the current tenant.
 *
 * @example POST /api/http/media/trash
 *
 * Features:
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!(isAdmin || isOwner)) {
      logger.warn(
        `Access denied for trash: User ${user._id} attempted to trash media ${mediaItem._id} owned by ${ownerId}`,
      );
