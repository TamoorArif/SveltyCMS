/**
 * @file src/routes/api/http/system-settings/export/+server.ts
...
async function exportSettings(options: ExportOptions): Promise<{
  ...
  logger.info(
    `Exported ${Object.keys(settings).length} regular settings, ${Object.keys(sensitive).length} sensitive settings`,
  );
