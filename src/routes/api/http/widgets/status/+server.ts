/**
 * @file src/routes/api/http/widgets/status/+server.ts
...
export const POST = apiHandler(async ({ locals, request }) => {
  ...
  try {
    ...
    if (!hasWidgetPermission) {
      logger.warn(
        `User ${user._id} (role: ${user.role}) denied access to API /api/http/widgets due to insufficient role permissions`,
      );
    ...
    if (!allWidgetsResult.success) {
      throw new AppError(
        `Failed to fetch widgets: ${allWidgetsResult.error?.message || "Unknown error"}`,
        500,
    ...
    if (!isActive) {
      ...
      for (const schema of allCollections) {
        ...
        if (
          ...
          schema.fields.some((field: any) => (field as any).widget === widgetName)
        ) {
    ...
    if (!(updateResult.success && updateResult.data)) {
      const errorMsg =
        !updateResult.success && "error" in updateResult
          ? (updateResult.error as { message?: string })?.message
          : undefined;
      ...
      throw new AppError(
        `Failed to update widget status: ${errorMsg || "Unknown error"}`,
        500,
        "UPDATE_FAILED",
      );
    }
    ...
    logger.debug("[/api/http/widgets/status] Clearing widget active cache and related collections", {
    ...
    try {
      ...
      await cacheService.clearByPattern("api:*:/api/http/user*"); // Admin UI may show user data
    } catch (cacheError) {
      logger.warn(
        "[/api/http/widgets/status] Enhanced cache clearing failed (non-critical):",
        cacheError,
      );
    }
