/**
 * @file src/databases/mariadb/modules/system/jobs-module.ts
 * @description Background jobs module for MariaDB.
 */

import { and, eq, lte, sql } from 'drizzle-orm';
import type { DatabaseId, DatabaseResult, EntityCreate, Job, PaginationOption } from '../../../db-interface';
...
export class JobsModule {
	...
	async create(job: EntityCreate<Job>): Promise<DatabaseResult<Job>> {
		return this.core.wrap(async () => {
			...
			const [result] = await this.db!.select().from(schema.sveltyJobs).where(eq(schema.sveltyJobs._id, id));
	...
	async getNextReady(limit = 10, tenantId?: string | null): Promise<DatabaseResult<Job[]>> {
		return this.core.wrap(async () => {
			const conditions = [eq(schema.sveltyJobs.status, 'pending'), lte(schema.sveltyJobs.nextRunAt, new Date())];
			if (tenantId) {
	...
	async list(options?: PaginationOption & { status?: string; taskType?: string }): Promise<DatabaseResult<Job[]>> {
		return this.core.wrap(async () => {
	...
	async count(filter?: Record<string, unknown>): Promise<DatabaseResult<number>> {
		return this.core.wrap(async () => {
			let q = this.db!.select({ count: sql<number>`count(*)` }).from(schema.sveltyJobs).$dynamic();
			const conditions = [];
	...
	async cleanup(olderThan: Date): Promise<DatabaseResult<number>> {
		return this.core.wrap(async () => {
			const result = await this.db!.delete(schema.sveltyJobs).where(lte(schema.sveltyJobs.createdAt, olderThan));
			return (result as any).affectedRows || (result as any).count || 0;
		}, 'JOB_CLEANUP_FAILED');
	}
}
