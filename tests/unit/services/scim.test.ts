...
describe('SCIM PATCH Engine', () => {
	...
	it('should handle replace operation with path', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'replace', path: 'active', value: false }]);
		expect(updates).toEqual({ isActive: false });
	});

	it('should handle add operation with path', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'add', path: 'displayName', value: 'John Doe' }]);
	...
	it('should handle replace without path (merge object)', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'replace', value: { userName: 'jane@acme.com', active: false } }]);
	...
	it('should handle name object in no-path merge', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'replace', value: { name: { givenName: 'Jane', familyName: 'Smith' } } }]);
		expect(updates.username).toBe('Jane');
		expect(updates.lastName).toBe('Smith');
	});

	it('should handle emails array in no-path merge', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'replace', value: { emails: [{ value: 'new@example.com', primary: true }] } }]);
		expect(updates.email).toBe('new@example.com');
	});

	it('should ignore unknown paths', () => {
		const updates = applyScimPatchOps(existingUser, [{ op: 'replace', path: 'unknownField', value: 'test' }]);
