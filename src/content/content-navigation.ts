...
export const contentNavigation = {
	...
	async getContentStructure(): Promise<ContentNode[]> {
		if (contentStructure.initState === 'initializing') {
			logger.warn('[ContentNavigation] getContentStructure called during initialization, returning empty array');
			return [];
		}
	 ...
	 */
	getNavigationStructureProgressive(options?: { maxDepth?: number; expandedIds?: Set<string>; tenantId?: string | null }): NavigationNode[] {
	...
	getNodePath(nodeId: string): ContentNode[] {
		...
		while (currentNode) {
			...
			currentNode = currentNode.parentId ? contentStructure.getNode(currentNode.parentId) : undefined;
		}
