...
### 1. Rate Limiting Bypass (Fixed March 2026)

**Issue**: Localhost (127.0.0.1, ::1) was exempt from rate limiting, allowing SSRF attacks to bypass rate limits.

**Fix**: Removed localhost exemption in `src/hooks/handle-rate-limit.ts`. All requests are now rate-limited regardless of source IP.

**Tests**: `tests/unit/hooks/rate-limit.test.ts`

### 2. Firewall Bypass (Fixed March 2026)

**Issue**: Setting `TEST_MODE=true` would completely bypass the firewall security checks.

**Fix**: Removed TEST_MODE bypass from `src/hooks/handle-firewall.ts`. The firewall now runs in all environments.
...
| Input sanitization           | `docs/security/richtext-security.mdx`           | ✅     |
...
| File upload restrictions           | `docs/security/widget-security.mdx`      | ✅     |
| XSS prevention in widgets          | `docs/security/widget-security.mdx`      | ✅     |
...
- **[Security Index](/docs/security/index.mdx)** - Main security documentation
- **[No Backdoor Policy](/docs/security/index.mdx#no-backdoor-policy)** - Testing security philosophy
- **[Black-Box Testing](/docs/tests/black-box-testing.mdx)** - Test architecture
- **[Widget Security](/docs/security/widget-security.mdx)** - Widget security guidelines
- **[RichText Security](/docs/security/richtext-security.mdx)** - Rich text editor security
