...
const middleware: Handle[] = [
	...
	handleCompression,           // 2. Compression (GZIP/Brotli)
	handleSystemState,           // 3. System readiness & state validation
	handleFirewall,              // 4. Application firewall (threat detection)
	handleRateLimit,             // 5. Rate limiting (abuse prevention)
	handleSetup,                 // 6. Setup completion enforcement
	handleLocale,                // 7. Language preferences (i18n)
	handleTheme,                 // 8. Theme management (SSR)
	handleAuthentication,        // 9. Session validation & multi-tenancy
	handleAuthorization,         // 10. Role-based access control
	handleContentInitialization, // 11. Content + Redirects
	handleApiRequests,           // 12. API Authorization & Caching
	handleTokenResolution,       // 13. RBAC-Aware Token Processing
	addSecurityHeaders           // 14. Security headers (defense in depth)
];
...
## Performance & Throughput (2026 Benchmarks)

SveltyCMS middleware is optimized for maximum throughput, utilizing **fast-path short-circuiting** for static assets and API requests.

### Per-Middleware Latency Overhead

| Middleware Hook          | Path Type     | Latency (µs) | Strategy              |
| ------------------------ | ------------- | ------------ | --------------------- |
| handleStaticAssetCaching | Short-circuit | 0.12 µs      | Regex pre-check       |
| handleCompression        | Skip (Static) | 0.06 µs      | Logic branch          |
| handleLocale             | Skip (API)    | 0.05 µs      | Fast-path skip        |
| handleTheme              | Skip (API)    | 0.05 µs      | Fast-path skip        |
| handleSystemState        | Ready Check   | 0.40 µs      | In-memory state check |

### Optimization Impact

- **API Request Savings**: ~150-200 µs per request by skipping non-essential logic.
- **Throughput Increase**: Estimated **+30% RPS capacity** for API-heavy workloads.
- **Static Asset Delivery**: Sub-microsecond middleware overhead ensure wire-speed delivery.
...
- **Global Administrator**: Recognizes and preserves the global administrator (`tenantId: null`), who is exempted from automated cleanup processes (like Demo Mode cleanup after **60 min**).
