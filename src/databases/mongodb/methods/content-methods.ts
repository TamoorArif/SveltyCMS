...
export function buildTree(nodes: ContentNode[]): ContentNode[] {
  ...
  logger.trace(
    `[buildTree] Built tree with ${roots.length} root nodes from ${nodes.length} total nodes`,
  );
  return roots;
}
...
export class MongoContentMethods {
  ...
  async upsertNodeByPath(
    nodeData: Omit<ContentNode, "_id" | "createdAt" | "updatedAt">,
  ): Promise<DatabaseResult<ContentNode>> {
    try {
        ...
        if ("parentId" in normalizedChanges) {
          ...
          if (normalizedParentId === null) {
            if (originalParentId !== null && originalParentId !== undefined) {
              logger.warn(
                `[bulkUpdateNodes] Unable to safely normalize parentId for path="${path}". Falling back to null value.`,
                {
    ...
    try {
      ...
      for (const { path, expectedId, changes } of expectedNodes) {
        ...
        if (existing) {
          ...
          if (existingId !== expectedId) {
            ...
            logger.info(
              `[fixMismatchedNodeIds] Queuing fix for node at path="${path}": ${existingId} → ${expectedId}`,
            );
              ...
              insertOne: {
                document: {
                  ...
                  createdAt: ((existing as any).createdAt ||
                    new Date().toISOString()) as unknown as ISODateString,
                  updatedAt: new Date().toISOString() as unknown as ISODateString,
                },
              },
            });
      ...
      return {
        ...
        error: createDatabaseError(
          error,
          "NODE_FIX_IDS_ERROR",
          "Failed to fix mismatched node IDs.",
        ),
      };
    }
  }
  ...
  async createDraft(
    draft: Omit<ContentDraft, "_id" | "createdAt" | "updatedAt">,
  ): Promise<DatabaseResult<ContentDraft>> {
    return this.draftsRepo.insert(draft);
  }

  async getDraftsForContent(
  ...
  async createRevision(
    revision: Omit<ContentRevision, "_id" | "createdAt">,
  ): Promise<DatabaseResult<ContentRevision>> {
    return this.revisionsRepo.insert(
      revision as Omit<ContentRevision, "_id" | "createdAt" | "updatedAt">,
    );
  }
  ...
  async getRevisionHistory(
    ...
    try {
      ...
      const totalRes = await this.revisionsRepo.count(
