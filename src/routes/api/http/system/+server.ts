/** @file src/routes/api/http/system/+server.ts @description System Management API - unified database-agnostic endpoint for system operations features: [health monitoring, reinitialization, service restart, backup trigger, cache clearing] */
...
/**
 * GET /api/http/system?action=health
  ...
  try {
    switch (action) {
      case "health": {
        ...
        const statusCode =
          healthReport.overallStatus === "READY" || healthReport.overallStatus === "DEGRADED"
            ? 200
            : 503;
  ...
  switch (action) {
    ...
    case "restart-service": {
      ...
      if (!validServices.includes(serviceName)) {
        throw new AppError(
          `Invalid service name. Valid services: ${validServices.join(", ")}`,
          400,
          "INVALID_SERVICE",
        );
      }
