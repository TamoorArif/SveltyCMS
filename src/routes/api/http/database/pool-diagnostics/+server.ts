 ...
 * GET /api/http/database/pool-diagnostics
 *
...
export const GET = apiHandler(async ({ locals }) => {
  ...
  if (!db.monitoring?.performance || typeof db.monitoring.performance.getMetrics !== "function") {
    ...
    throw new AppError(
      "Pool diagnostics not implemented for this database adapter",
      501,
      "NOT_IMPLEMENTED",
    );
  }
  ...
  if (!metricsResult.success) {
    throw new AppError(
      metricsResult.message || "Failed to retrieve pool diagnostics",
      500,
      "METRICS_ERROR",
    );
  }
