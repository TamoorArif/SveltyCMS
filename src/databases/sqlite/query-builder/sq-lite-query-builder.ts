/**
 * @file src/databases/sqlite/queryBuilder/sq-lite-query-builder.ts
 * @description SQLite implementation of the QueryBuilder interface using Drizzle ORM
 *
 * Features:
 * - Where conditions
 * - In conditions
...
export class SQLiteQueryBuilder<T extends BaseEntity> implements QueryBuilder<T> {
  ...
  constructor(adapter: SQLiteAdapter, collection: string) {
  ...
  async findOne(): Promise<DatabaseResult<T | null>> {
    ...
    try {
      ...
      return {
        success: true,
        data: result
          ? (utils.convertDatesToISO(result as Record<string, unknown>) as unknown as T)
          : null,
        meta: { executionTime: Date.now() - startTime },
      };
    } catch (error) {
  ...
  async deleteMany(): Promise<DatabaseResult<{ deletedCount: number }>> {
    ...
    try {
      let q = this.db
        .delete(this.table as unknown as import("drizzle-orm/sqlite-core").SQLiteTable)
        .$dynamic();
      if (this.conditions.length > 0) {
        q = q.where(and(...this.conditions));
      }
