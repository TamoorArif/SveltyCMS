...
- `/api/system-settings/*` - System settings export/import endpoints
- `/api/import/full` - Import endpoint
...
```http
POST /api/system-settings/export
```
...
### 3. Batch Settings Export

Exports values for all accessible configuration groups.
...
```mermaid
graph TD
    A[Client: POST /api/import-data] --> B{Valid Request?}
    B -- No --> C[Return 400/422 Error]
    B -- Yes --> D{Items > 50 or async=true?}

    D -- "Yes (Async)" --> E[Dispatch to JobQueue]
    E --> F[Return jobId + Status: 'pending']
    F --> G[Background: Process in Chunks]
    G --> H[Final: Job Completed]

    D -- "No (Sync)" --> I[Immediate Processing]
    I --> J[Return Results + Status: 'completed']
```

### 1. Import Data

Imports data into collections from JSON. Large datasets (>50 items) are automatically processed in the background with enhanced enterprise features.
...
- `collectionName` (string, required) - The name of the collection to import into.
- `data` (array, required) - Array of document objects to import.
- `mode` (string, optional) - `merge` (default) or `replace` (clears collection first).
- `duplicateStrategy` (string, optional) - `skip` (default) or `overwrite` (requires `_id`).
- `async` (boolean, optional) - Force background processing. Defaults to `true` if `data.length > 50`.

**Enterprise Features (March 2026):**

- **Progress Tracking**: Background jobs update their `progress` field (0-100) and `metadata` (imported/skipped/errors) every 100 items.
- **Temp Store Backing**: Datasets larger than 1000 items are automatically offloaded to a temporary file-store to prevent database index bloat and payload limits.
- **Concurrency Guard**: The system respects `IMPORT_CONCURRENT_MAX` (default: 5) to prevent resource exhaustion during simultaneous large imports.
...
Returned for small datasets processed immediately.
...
**Asynchronous Success (200):**
Returned for large datasets or when `async: true` is provided.
...
### 3. External Smart Import (AI-Powered)

Migrates content from external platforms like Drupal or WordPress.
...
```json
{
  "sourceType": "drupal", // or "wordpress"
  "sourceUrl": "https://old-site.com",
  "apiKey": "Bearer ...",
  "sourceTypeIdentifier": "article",
...
- Cross-tenant exports restricted to super-admins in multi-tenant mode

### Data Encryption

- Default AES-256-GCM encryption for all sensitive fields (API keys, secrets) in both standard and full exports
- Secure Argon2id key derivation
- Encrypted backup storage
- Password-protected full exports (optional)
...
curl -X POST https://old.example.com/api/system-settings/export > backup.json.enc

# Import to PostgreSQL instance
curl -X POST https://new.example.com/api/system-settings/import \
  -d @backup.json.enc
```
...
curl -X POST https://production.example.com/api/import-data \
  -d @posts.json
```
...
```bash
# Export tenant data
curl -X POST "https://cms.example.com/api/system-settings/export?tenantId=tenant-a" \
  > tenant-a-backup.json.enc

# Import to new tenant
curl -X POST https://new-cms.example.com/api/system-settings/import \
...
const exportResponse = await fetch("/api/export/full", {
  ...
  body: JSON.stringify({ includeCollections: true }),
});
...
// Import data
const importResponse = await fetch("/api/system-settings/import", {
  method: "POST",
  credentials: "include",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
- [Collection System](/docs/api/collection-api.mdx)
- [Settings Management](/docs/api/settings-api.mdx)
...
- `src/routes/api/system-settings/export/+server.ts` - Full export endpoint
- `src/routes/api/system-settings/import/+server.ts` - Full import endpoint
- `src/routes/api/import-data/+server.ts` - Import endpoint
