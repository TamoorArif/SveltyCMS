/**
 * @file src/routes/api/http/marketplace/widgets/+server.ts
...
export const GET = apiHandler(async ({ url, locals }) => {
  try {
    ...
    if (!hasWidgetPermission) {
      logger.warn(
        `User ${user._id} denied access to marketplace API due to insufficient permissions`,
      );
    ...
    logger.trace(
      `Retrieved ${widgets.length} marketplace widgets (filtered: category=${category}, search=${search})`,
    );
