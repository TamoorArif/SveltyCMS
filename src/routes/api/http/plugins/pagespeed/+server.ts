/**
 * @file src/routes/api/http/plugins/pagespeed/+server.ts
...
import {
  ...
  getCachedResult,
  storeResult,
} from "@src/plugins/pagespeed/service";
...
export const POST = apiHandler(async ({ request, locals }) => {
  ...
  try {
    ...
    if (!(entryId && collectionId && language)) {
      throw new AppError(
        "Missing required parameters: entryId, collectionId, language",
        400,
        "MISSING_PARAMS",
      );
    }
    ...
    if (device !== "mobile" && device !== "desktop") {
      throw new AppError(
        'Invalid device type. Must be "mobile" or "desktop"',
        400,
        "INVALID_DEVICE",
      );
    }
    ...
    if (!url) {
      throw new AppError(
        "Could not derive URL for entry. Ensure entry has a slug field.",
        400,
        "URL_DERIVATION_FAILED",
      );
    }
    ...
    if (!metrics) {
      throw new AppError(
        "Failed to fetch PageSpeed Insights. Check API key configuration.",
        500,
        "API_ERROR",
      );
    }
