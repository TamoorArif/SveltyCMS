/**
 * @file src/routes/api/http/user/update-user-attributes/+server.ts
 ...
 *
 * Usage:
 * PUT /api/http/user/update-user-attributes
...
export const PUT: RequestHandler = apiHandler(async ({ request, locals, cookies }) => {
  ...
  if (getPrivateSettingSync("MULTI_TENANT") && !isEditingSelf) {
    ...
    if (!userToUpdate || userToUpdate.tenantId !== tenantId) {
      ...
      throw new AppError(
        "Forbidden: You can only edit users within your own tenant.",
        403,
        "FORBIDDEN_TENANT",
      );
    }
  ...
  if (newUserData.role) {
    if (isEditingSelf) {
      ...
      logger.warn("User attempted to change their own role.", {
        userId: user._id,
        attemptedRole: newUserData.role,
      });
      throw new AppError(
