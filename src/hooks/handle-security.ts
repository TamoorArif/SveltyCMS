...
export const handleSecurity: Handle = async ({ event, resolve }) => {
	...
	if (isStaticAsset(url.pathname) || (isLocalhost(clientIp) && dev && request.headers.get('x-test-security') !== 'true')) {
	...
	try {
		...
		if (securityStatus.action === 'block') {
			...
			logger.warn(`Security block triggered: ${securityStatus.reason}`, { ip: clientIp, url: url.pathname });
...
function getClientIp(event: RequestEvent): string {
	try {
		...
		return event.request.headers.get('x-forwarded-for')?.split(',')[0].trim() || event.request.headers.get('x-real-ip') || '127.0.0.1';
	}
}
...
function isStaticAsset(pathname: string): boolean {
	return pathname.startsWith('/static/') || pathname.startsWith('/_app/') || STATIC_EXTENSIONS.test(pathname);
}
