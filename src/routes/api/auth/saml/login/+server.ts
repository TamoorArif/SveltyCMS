...
export const GET = apiHandler(async ({ url }) => {
	...
	if (isMultiTenant && tenant === 'default') {
		throw new AppError('Tenant identifier is required in multi-tenant mode', 400, 'TENANT_REQUIRED');
	}
	...
	if (!redirectUrl) {
		throw new AppError('Failed to generate SAML redirect URL. Ensure IdP is configured for this tenant.', 404, 'SAML_NOT_CONFIGURED');
	}
