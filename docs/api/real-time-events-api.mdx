...
| `yjs:sync`        | Binary document update (Yjs).    | `{ docId: string, updateBase64: string, origin: string }` |
| `webhook:failure` | An automated webhook failed.     | `{ collection: string, url: string }`     |

---

## Binary Document Synchronization (Yjs)

For multi-user editing, SveltyCMS uses the `yjs:sync` event to broadcast binary document updates.

### Workflow

1.  **Handshake**: The client fetches the initial state via `GET /api/collaboration/yjs?docId=...`.
2.  **Upstream**: Local changes are batched and sent via `POST /api/collaboration/yjs`.
3.  **Downstream**: The server applies the update and broadcasts it to all other clients via the SSE stream as a `yjs:sync` event.

### Throttling & Batching
To maintain enterprise performance, `yjs:sync` events are aggressively batched (50ms window) and awareness (cursors) are throttled (100ms) to prevent network congestion.
...
- **Memory Hygiene**: Strict resource management ensures `setInterval` heartbeats and `EventBus` listeners are properly destroyed upon client disconnect, preventing memory leaks and Out of Memory (OOM) errors over long-running instances.
