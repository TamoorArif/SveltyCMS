...
> **📚 This unified guide replaces:** `installation.mdx`, `testing.mdx`, and `TESTING_GUIDE.md.old`
...
8. [Collection UUID Management](#collection-uuid-management)
   > See [Compilation Pipeline](./compilation-pipeline.mdx) for detailed architecture.
9. [Key Components](#key-components)
10. [State Machine](#state-machine)
11. [Compilation Pipeline](./compilation-pipeline.mdx)
...
│   ├── private.ts             # Database credentials (auto-generated)
│   ├── roles.ts               # Role definitions
│   └── collections/           # Collection schemas (user-created)
├── compiledCollections/        # Compiled collections (auto-generated)
│   ├── Collections/
│   └── Menu/
├── docs/                       # Documentation
├── scripts/                    # Automation scripts
│   ├── seed-cms.js            # Database seeding script
...
│       ├── playwright.yml     # Test workflow
│       └── auto-release.yaml  # Release workflow
├── package.json
├── svelte.config.js
├── tailwind.config.ts
...
| `contentManager`  | Public system facade         | `src/content/index.ts`            |
...
for (const schema of collections) {
  await dbAdapter.collection.createModel(schema);
  // ⚠️ Potential race condition here
}
```
...
- ✅ **E2E Tests (Playwright)** - Full user workflows, browser automation
- ✅ **Integration Tests** - API endpoints, database operations
- ✅ **Unit Tests (Bun)** - Components, utilities, functions
- ✅ **CI/CD Automation** - GitHub Actions for testing and releases

### Test Coverage

- ✅ Authentication flows (Email, OAuth, 2FA)
- ✅ User management (CRUD, permissions, roles)
- ✅ Collection management (Builder, CRUD, relationships)
- ✅ All 95+ API endpoints tested
...
├── mocks/
│   └── setup.ts       # Preload file (mocks SvelteKit)
...
🧪 **Comprehensive Testing** - 95+ API endpoints, full user workflows  
🤖 **Automated CI/CD** - GitHub Actions for testing and releases  
📝 **Automated Scripts** - `seed-cms.js` and `create-admin.js` for CI/CD
...
- [Setup Actions](/docs/api/setup-actions-api.mdx) - Complete setup API reference
- [Collection API](/docs/api/collection-api.mdx) - Collection CRUD operations
- [Authentication API](/docs/api/authentication-2fa-api.mdx) - User authentication

### Database

- [Core Infrastructure](/docs/architecture/database/core-infrastructure.mdx) - Database adapter system
- [MongoDB Methods](/docs/architecture/database/mongodb-methods.mdx) - MongoDB-specific implementation
