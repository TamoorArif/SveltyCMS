---
path: "docs/api/token-api-endpoints.mdx"
...
| -------- | ------------------------- | --------------------------------------- | ------------- |
| `POST`   | `/api/token/create-token` | Create new invitation token             | ✅ Admin      |
...
```bash
curl -X POST https://cms.example.com/api/token/create-token \
...
```javascript
const response = await fetch("/api/token/create-token", {
  method: "POST",
  headers: {
...
```javascript
const result = await fetch("/api/token/create-token", {
  method: "POST",
...
- [Multi-Tenant Architecture](/docs/architecture/configuration-management.mdx)
