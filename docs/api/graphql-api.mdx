...
SveltyCMS employs robust, native GraphQL security features to prevent Denial of Service (DoS) and resource exhaustion attacks, without relying on external dependencies.

### Server-Side Architecture (Vite+)

The GraphQL engine is integrated into the **Vite+** middleware pipeline, ensuring high performance and security:

- **AST Validation**: Depth and alias limiting to prevent DoS.
- **Envelop Plugins**: JIT execution for sub-millisecond resolution.
- **Adapter Context**: Direct access to `dbAdapter` for efficient nested resolution (e.g. `Relation` widgets).
