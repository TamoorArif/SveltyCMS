...
const { PATCH: PATCH_ENTRY, DELETE: DELETE_ENTRY } =
  await import("@src/routes/api/collections/[collectionId]/[entryId]/+server");
...
describe("Collections API Unit Tests", () => {
  ...
  beforeEach(async () => {
    ...
    mockGetPrivateSettingSync =
      settingsModule.getPrivateSettingSync as unknown as typeof mockGetPrivateSettingSync;
      ...
      mockReplaceTokens.mockImplementation((text: string) => {
        if (text.includes("{{")) {
          return Promise.resolve(
            text.replace("{{user.name}}", "John").replace("{{user.email}}", "john@example.com"),
          );
        }
