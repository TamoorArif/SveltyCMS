...
export const DELETE = apiHandler(async ({ params, locals }) => {
	...
	try {
		...
		if (!exists) {
			logger.warn(`Unauthorized webhook delete attempt for ID ${id} by user ${locals.user._id} in tenant ${locals.tenantId}`);
...
export const PATCH = apiHandler(async ({ params, request, locals }) => {
	...
	try {
		...
		if (!exists) {
			logger.warn(`Unauthorized webhook update attempt for ID ${id} by user ${locals.user._id} in tenant ${locals.tenantId}`);
			throw new AppError('Webhook not found or access denied', 404, 'NOT_FOUND');
		}
		...
		logger.info(`Webhook updated: ${webhook.name} (${id}) for tenant ${locals.tenantId} by ${locals.user.email}`);
