/**
 * @file src/routes/api/http/media/delete/+server.ts
 * @description
 * API endpoint for deleting a media file within the current tenant.
 *
 * @example DELETE /api/http/media/delete
 *
 * Features:
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    if (!(isAdmin || isOwner)) {
      logger.warn(
        `Access denied for delete: User ${user._id} attempted to delete media ${mediaItem._id} owned by ${ownerId}`,
      );
