...
export function createConfirmModal(config: ActionModalConfig, onConfirm: () => void, onCancel?: () => void): ModalSettings {
	...
	return {
		...
		title: config.title,
		body: config.body,
		buttonTextConfirm: config.confirmText || button_confirm(),
		buttonTextCancel: config.cancelText || button_cancel(),
	...
	return createConfirmModal(
		{
			title,
			body: body + warning,
			confirmText: button_confirm?.() || 'Confirm',
			cancelText: button_cancel?.() || 'Cancel',
...
export function createScheduleModal(itemType: string, itemName: string, onConfirm: (scheduledDate: Date) => void): ModalSettings {
	return {
		type: 'component',
		title: scheduler_title?.({ type: itemType }) || 'Schedule Publication',
		body: scheduler_body?.({ name: itemName }) || 'Set a date and time to publish this entry.',
		component: {
			ref: 'DatePickerModal',
			props: {
				itemName,
		...
		buttonTextCancel: button_cancel(),
