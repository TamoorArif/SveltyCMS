...
export const GET = apiHandler(async ({ locals, url }) => {
  ...
  const userIds = [
    ...new Set(
      limitedEntries
        .map((entry) => entry.createdBy || entry.author || entry.creator)
        .filter(Boolean),
    ),
  ] as DatabaseId[];
