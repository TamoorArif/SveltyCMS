...
3. **Passing data via page.data**
   ...
   ```typescript
   // ✅ SAFE - Controlled data flow from server to client
   ...
   // +page.svelte (client)
   export let data;
   console.log(data.isConnected); // ✅ Safe
   ```
...
Found 2 critical security issue(s):
  ...
  ✗ src/routes/(app)/admin/+page.svelte:12
    ...
    → const secret = getPrivateSetting('JWT_SECRET');
...
export const load: PageServerLoad = async () => {
  // ✅ Server-only: privateEnv never sent to client
...
- [Authentication System](./01-authentication/authentication-system.mdx) - 3-Layer Security Architecture
- [Settings System](../user-guide/admin-user-management.md) - Managing system settings
- [Environment Variables](./00-installation/environment-variables.md) - Static vs dynamic settings
...
1. ✅ **Build-Time Validation**: Static analysis prevents vulnerabilities before deployment
2. ✅ **Runtime Threat Detection**: Real-time analysis with automated response
3. ✅ **Nonce-Based CSP**: Cryptographically secure XSS prevention
4. ✅ **Automated Response**: Dynamic threat mitigation without manual intervention
5. ✅ **Enterprise Monitoring**: Real-time security dashboard with detailed analytics
