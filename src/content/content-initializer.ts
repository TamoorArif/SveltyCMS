...
export const contentInitializer = {
  ...
  async initialize(
    ...
    if (initializedTenants.has(tenantId || "")) {
      ...
      if (initializedInSetupMode && isSetupComplete()) {
        logger.info(
          "[ContentInitializer] Setup completed after previous initialization. Forcing re-initialization...",
          { tenantId },
        );
    ...
    if (existingPromise) {
      logger.debug("[ContentInitializer] Waiting for existing initialization to complete", {
        tenantId,
      });
  ...
  async _doInitialize(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logger.trace(`[ContentInitializer] initialization attempt ${attempt}/${maxRetries}`, {
          tenantId,
        });
        ...
        if (skipReconciliation && setupComplete && (await contentCache.loadFromCache(tenantId))) {
          ...
          logger.info(
            `🚀 [ContentInitializer] initialized from cache in ${contentMetrics.getMetrics().initializationTime.toFixed(2)}ms`,
          );
          return;
        }
        ...
        contentMetrics.setInitializationTime(performance.now() - startTime);
        logger.info(
          `📦 [ContentInitializer] fully initialized in ${contentMetrics.getMetrics().initializationTime.toFixed(2)}ms`,
        );
        return;
      } catch (error) {
        lastError = error instanceof Error ? error : new Error(String(error));
        logger.warn(
          `[ContentInitializer] Initialization attempt ${attempt} failed:`,
          lastError.message,
        );
  ...
  async refresh(tenantId: string | null = null, skipReconciliation = false): Promise<void> {
    logger.info(
      `[ContentInitializer] Refreshing content state${skipReconciliation ? " (fast/skip-reconcile)" : ""}...`,
      { tenantId },
    );
    contentStructure.initState = "initializing";
    contentCache.clear();
