...
export const contentReconciler = {
	 ...
	 */
	async reconcileAndBuildStructure(allSchemas: Schema[], tenantId?: string | null, skipReconciliation = false, adapter?: IDBAdapter): Promise<void> {
		...
		if (!dbAdapter) {
			logger.info('[ContentReconciler] No database available (setup mode) - building structure from files only');
		...
		if (skipReconciliation) {
			if (dbNodes.length === 0) {
				logger.warn('[ContentReconciler] ⚠️ Skip reconciliation requested, but DB is EMPTY! Forcing reconciliation.');
				skipReconciliation = false;
			} else {
				logger.info(`[ContentReconciler] Skipping reconciliation (trusting DB state with ${dbNodes.length} nodes).`);
			}
		}
		...
		if (skipReconciliation) {
			...
			for (const op of operations) {
				...
				if (existingNode && existingNode._id.toString() !== op._id.toString()) {
					logger.warn(`[ContentReconciler] ID Mismatch for path="${op.path}": DB=${existingNode._id} vs Schema=${op._id}.`);
	...
	async _loadFinalStructure(dbAdapter: IDBAdapter, operations: ContentNode[], tenantId?: string | null, dbNodes?: ContentNode[]): Promise<void> {
		const result =
	...
	async getContentStructureFromDatabase(format: 'flat' | 'nested' = 'nested', tenantId?: string | null): Promise<ContentNode[]> {
		...
		if (!result.success) {
			logger.error('[ContentReconciler] Failed to get content structure from database:', result.error);
			return [];
		}
	 ...
	 */
	async upsertContentNodes(operations: import('./types').ContentNodeOperation[], tenantId?: string | null): Promise<ContentNode[]> {
		const dbAdapter = await getDbAdapter();
		...
		for (const operation of operations) {
			...
			switch (type) {
				case 'create': {
					...
					const { createdAt, updatedAt, ...createFields } = node as any;
		...
		if (bulkUpdates.length > 0) await dbAdapter.content.nodes.bulkUpdate(bulkUpdates, { tenantId, bypassTenantCheck: true });

		const result = await dbAdapter.content.nodes.getStructure('flat', { tenantId, bypassTenantCheck: true });
	 ...
	 */
	async reorderContentNodes(operations: import('./types').ContentNodeOperation[], tenantId?: string | null): Promise<ContentNode[]> {
		const dbAdapter = await getDbAdapter();
		if (!dbAdapter) throw new Error('Database adapter not available');
	...
	async moveNodeWithDescendants(nodeId: string, newParentId: string | undefined): Promise<void> {
		...
		await dbAdapter.content.nodes.bulkUpdate([{ path: node.path as string, changes: { parentId: node.parentId, updatedAt: node.updatedAt } }], {
