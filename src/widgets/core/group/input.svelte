...
function getWidgetLoader(widgetName: string) {
	...
	const camelFn =
		widgets.widgetFunctions[
			widgetName.charAt(0).toLowerCase() + widgetName.slice(1)
		];
	const camelPath =
		(camelFn as any)?.componentPath || (camelFn as any)?.inputComponentPath;
	...
	const lowerPath =
		(lowerFn as any)?.componentPath || (lowerFn as any)?.inputComponentPath;
...
const variant = $derived(
	variantClasses[(field as any).variant as keyof typeof variantClasses] ||
...
let isCollapsed = $derived(
	manualCollapsed ?? (field as any).collapsed ?? false,
);
