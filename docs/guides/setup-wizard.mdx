...
sequenceDiagram
    ...
    participant Svr as Server Action
    ...
    Svr->>DB: Create admin user
    Svr->>DB: Create session
    Svr->>Sys: Initialize system
    Sys->>Sys: Load settings
    Sys->>Sys: Register collections
    Svr->>Svr: Pre-warm cache (CollectionService)
    Svr-->>W: Success + session cookie
    W->>W: Clear localStorage
    W->>W: Show success toast
    W->>U: Redirect to dashboard (Immediate)
```
