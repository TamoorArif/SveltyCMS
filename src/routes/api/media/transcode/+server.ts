/**
 * @file src/routes/api/media/transcode/+server.ts
 * @description
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		// Note: In a real 2026 production env, this would be delegated to a background worker
		// but for the DXP core, we implement the direct ffmpeg logic.
		if (targetFormat === 'hls') {
			const hlsCommand = `ffmpeg -i "${inputPath}" \
				-filter_complex "[0:v]split=2[v1][v2];[v1]scale=w=1280:h=720[v1out];[v2]scale=w=1920:h=1080[v2out]" \
				-map "[v1out]" -c:v:0 libx264 -b:v:0 2800k \
				-map "[v2out]" -c:v:1 libx264 -b:v:1 5000k \
				-map a:0 -c:a aac -b:a 128k \
				-f hls -hls_time 10 -hls_playlist_type event \
				-master_pl_name master.m3u8 \
				-hls_segment_filename "${outputDir}/file_%v_%03d.ts" \
