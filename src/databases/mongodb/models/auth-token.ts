...
export class TokenAdapter {
  ...
  constructor() {
    ...
    this.TokenModel =
      mongoose.models?.auth_tokens || mongoose.model<TokenDocument>("auth_tokens", TokenSchema);
  }
  ...
  async consumeToken(
    ...
    try {
      ...
      const tokenDoc = await this.TokenModel.findOneAndDelete(
        query as QueryFilter<TokenDocument>,
      ).lean();
      if (!tokenDoc) {
  ...
  async deleteTokens(
    ...
    try {
      ...
      const valueResult = await this.TokenModel.deleteMany(
  ...
  async blockTokens(
    ...
    try {
      ...
      const valueResult = await this.TokenModel.updateMany(
        valueFilter as QueryFilter<TokenDocument>,
        {
          blocked: true,
        },
