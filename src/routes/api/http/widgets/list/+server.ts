/**
 * @file src/routes/api/http/widgets/list/+server.ts
...
export const GET = apiHandler(async ({ url, locals }) => {
  ...
  try {
    ...
    if (!activeWidgetsResult.success) {
      throw new AppError(
        `Failed to fetch active widgets: ${activeWidgetsResult.error?.message || "Unknown error"}`,
        500,
        "FETCH_ACTIVE_FAILED",
      );
    }
    ...
    logger.debug("[/api/http/widgets/list] Active widgets from database", {
      tenantId,
