...
export const GET = apiHandler(async ({ locals, url }) => {
	...
	if (getPrivateSettingSync('MULTI_TENANT')) {
		if (targetTenantId !== tenantIdFromLocals && !isSuperAdmin) {
			...
			throw new AppError('Unauthorized: You can only access automations for your own tenant.', 403, 'TENANT_MISMATCH');
		}
...
export const POST = apiHandler(async ({ request, locals }) => {
	...
	try {
		...
		logger.info(`Automation created: ${flow.name} (${flow.id}) for tenant ${tenantId} by ${locals.user.email}`);
