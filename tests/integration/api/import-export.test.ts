/**
 * @file tests/bun/api/import-export.test.ts
 * @description Integration tests for Data Import/Export API endpoints
 *
 * Tests data import/export endpoints:
 * - POST /api/export-data - Export collection data
 * - POST /api/import-data - Import collection data
 * - POST /api/export/full - Full system export
 * - POST /api/import/full - Full system import
 */

import { afterAll, beforeAll, beforeEach, describe, expect, it } from 'bun:test';
import { getApiBaseUrl, safeFetch, waitForServer } from '../helpers/server';
import { cleanupTestDatabase, prepareAuthenticatedContext } from '../helpers/test-setup';

const BASE_URL = getApiBaseUrl();

// Minimal valid metadata for full import
const MINIMAL_METADATA = {
	exported_at: new Date().toISOString(),
	cms_version: '0.0.7',
	export_id: 'test-export-id'
};

describe('Import/Export API', () => {
	let authCookie: string;

	beforeAll(async () => {
		await waitForServer();
	});

	afterAll(async () => {
		await cleanupTestDatabase();
	});

	beforeEach(async () => {
		authCookie = await prepareAuthenticatedContext();
	});

	describe('Export Collection Data', () => {
		it('should export collection data or return 404 for non-existent collection', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts'
				})
			});

			// Collection may not exist yet, so 404 is acceptable
			expect(response.status).toBe(response.ok ? 200 : 404);
		});

		it('should require collection name parameter', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({})
			});

			expect(response.status).toBe(400);
		});

		it('should require authentication for export', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ collectionName: 'Posts' })
			});

			expect(response.status).toBe(401);
		});

		it('should handle non-existent collections', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'NonExistentCollection'
				})
			});

			expect(response.status).toBe(404);
		});

		it('should support export format options', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					format: 'json'
				})
			});

			// Collection may not exist, so 404 is acceptable
			expect(response.status).toBe(response.ok ? 200 : 404);
		});

		it('should include metadata in export', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					includeMetadata: true
				})
			});

			// Collection may not exist
			expect(response.status).toBe(response.ok ? 200 : 404);
		});

		it('should support filtered exports', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					filter: { status: 'published' }
				})
			});

			// Collection may not exist
			expect(response.status).toBe(response.ok ? 200 : 404);
		});
	});

	describe('Import Collection Data', () => {
		it('should import collection data', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [
						{
							title: 'Imported Post',
							content: 'Content'
						}
					]
				})
			});

			expect(response.status).toBe(200);
		});

		it('should validate import data structure', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: 'invalid data'
				})
			});

			expect(response.status).toBe(422);
		});

		it('should require authentication for import', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					collectionName: 'Posts',
					data: []
				})
			});

			expect(response.status).toBe(401);
		});

		it('should support replace vs merge import mode', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [],
					mode: 'replace'
				})
			});

			expect(response.status).toBe(200);
		});

		it('should return import statistics', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [{ title: 'Post 1' }, { title: 'Post 2' }]
				})
			});

			expect(response.status).toBe(200);
			const result = await response.json();
			expect(result.success).toBe(true);
			expect(result.imported).toBeDefined();
			expect(result.total).toBeDefined();
		});

		it('should handle validation errors in import data', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [{ invalidField: 'value' }]
				})
			});

			expect(response.status).toBe(200); // Should return 200 with partial success or error info
		});

		it('should support duplicate handling strategies', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [],
					duplicateStrategy: 'skip'
				})
			});

			expect(response.status).toBe(200);
		});
	});

	describe('General Export', () => {
		it('should export data with general export endpoint', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					type: 'all'
				})
			});

			expect(response.status).toBe(200);
		});

		it('should support multiple export types', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					type: 'settings'
				})
			});

			expect(response.status).toBe(200);
		});

		it('should require admin authentication', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ type: 'all' })
			});

			expect(response.status).toBe(401);
		});

		it('should return downloadable export file', async () => {
			const response = await safeFetch(`${BASE_URL}/api/export`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					type: 'all',
					download: true
				})
			});

			expect(response.status).toBe(200);
			const contentType = response.headers.get('content-type');
			expect(contentType).toContain('application/json');
		});
	});

	describe('Full System Import', () => {
		it('should perform full system import', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					data: {
						metadata: MINIMAL_METADATA,
						settings: {},
						collections: []
					}
				})
			});

			expect(response.status).toBe(200);
		});

		it('should validate full import data structure', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					data: 'invalid'
				})
			});

			expect(response.status).toBe(400);
		});

		it('should require admin authentication for full import', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ data: { metadata: MINIMAL_METADATA } })
			});

			expect(response.status).toBe(401);
		});

		it('should support incremental vs full replace import', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					data: {
						metadata: MINIMAL_METADATA,
						settings: {}
					},
					options: {
						strategy: 'skip'
					}
				})
			});

			expect(response.status).toBe(200);
		});

		it('should return comprehensive import results', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					data: {
						metadata: MINIMAL_METADATA,
						settings: {}
					}
				})
			});

			expect(response.status).toBe(200);
			const result = await response.json();
			expect(result.success).toBe(true);
			expect(result.imported).toBeDefined();
		});

		it('should handle partial import failures gracefully', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import/full`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					data: {
						metadata: MINIMAL_METADATA,
						invalid: 'data'
					}
				})
			});

			// Should return 200 or 400 or 207 depending on implementation
			expect([200, 400, 207]).toContain(response.status);
		});
	});

	describe('Data Integrity', () => {
		it('should preserve relationships in export/import', async () => {
			const exportResponse = await safeFetch(`${BASE_URL}/api/export-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts'
				})
			});

			// Collection may not exist, both statuses are acceptable
			expect(exportResponse.status).toBe(exportResponse.ok ? 200 : 404);
		});

		it('should handle large datasets efficiently', async () => {
			const largeDataset = Array.from({ length: 100 }, (_, i) => ({
				title: `Post ${i}`,
				content: 'Content'
			}));

			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: largeDataset
				})
			});

			expect(response.status).toBe(200);
		});

		it('should validate data integrity after import', async () => {
			const response = await safeFetch(`${BASE_URL}/api/import-data`, {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					Cookie: authCookie
				},
				body: JSON.stringify({
					collectionName: 'Posts',
					data: [{ title: 'Test' }],
					validateIntegrity: true
				})
			});

			expect(response.status).toBe(200);
		});
	});
});
