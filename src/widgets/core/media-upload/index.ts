	...
	modifyRequest: async ({ data, field, user, tenantId, collectionName }) => {
		if (import.meta.env.SSR) {
			...
			if (value instanceof File) {
				...
				const savedMedia = await service.saveMedia(value, (user as any)._id.toString(), 'private', basePath);
				...
				for (const item of value) {
					if (item instanceof File) {
						const savedMedia = await service.saveMedia(item, (user as any)._id.toString(), 'private', basePath);
