/** @file src/routes/api/http/user/login/+server.ts @description Quantum-resistant user authentication API endpoint features: [Argon2id password verification, AES-256-GCM session tokens, multi-tenant awareness, blocked user protection, user enumeration prevention] */
...
export const POST = apiHandler(async ({ request, cookies, locals }) => {
  ...
  try {
    ...
    const userLookupCriteria: { email: string; tenantId?: string | null } = {
      email,
    };
    ...
    const session = await auth.createSession({
      ...
      expires: new Date(
        Date.now() + parseSessionDuration("1d"),
