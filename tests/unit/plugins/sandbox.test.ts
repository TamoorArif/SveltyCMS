...
describe("Plugin Sandbox - Scoped DB Access", () => {
  ...
  it("should block access to protected system collections", () => {
    ...
    expect(() => adapter.crud.findMany("users" as any, {} as any)).toThrow(
      /denied access to protected collection/,
    );
    expect(() => adapter.crud.findMany("sessions" as any, {} as any)).toThrow(
      /denied access to protected collection/,
    );
    expect(() => adapter.crud.findMany("tokens" as any, {} as any)).toThrow(
      /denied access to protected collection/,
    );
    expect(() => adapter.crud.findMany("roles" as any, {} as any)).toThrow(
      /denied access to protected collection/,
    );
    expect(() => adapter.crud.findMany("audit_logs" as any, {} as any)).toThrow(
      /denied access to protected collection/,
    );
  });
  ...
  it("should enforce query limit", () => {
    ...
    // 101st should throw
    expect(() => adapter.crud.findMany("one_too_many" as any, {} as any)).toThrow(
