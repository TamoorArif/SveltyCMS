...
1. ✅ Scans collection schema files
2. ✅ Validates field definitions
3. ✅ Generates TypeScript union types
4. ✅ Updates `src/content/types.ts` with new types
5. ✅ Provides IDE autocomplete for collection names
...
getCollection("InvalidName"); // ❌ TypeScript error
```
...
const collections: Record<ContentTypes, Schema> = {
  Posts: {
    /* ... */
  }, // ✅ Autocomplete suggests 'Posts', 'Users', etc.
  Users: {
    /* ... */
  },
...
const posts = getCollection("BlogPosts"); // ❌ TypeScript error - must update
const posts = getCollection("Articles"); // ✅ Fixed
```
...
- **`src/content/content-manager.ts`**: Thin coordinator (public API facade)
- **`src/content/content-structure.svelte.ts`**: Svelte 5 runes-based state
- **`src/content/content-initializer.ts`**: Lifecycle and initialization
- **`src/content/content-navigation.ts`**: Tree and navigation operations
- **`src/content/content-reconciler.ts`**: DB-agnostic file ↔ DB synchronization
- **`src/content/content-cache.svelte.ts`**: L1/L2 cache integration
- **`src/content/content-metrics.ts`**: Performance and health monitoring
- **`src/content/vite.ts`**: Vite plugin for type generation
- **`src/content/types.ts`**: Generated ContentTypes union and core interfaces
