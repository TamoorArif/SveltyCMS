...
3. **Verify CI Parity**: If code changes were made, run `bun run lint && bun run check && bun run test`
4. **Submit PR**: Create pull request with descriptive title
5. **Review**: Reviewers check for accuracy, clarity, and adherence to guidelines
6. **Update `updated` field**: Change the date in frontmatter
