/**
 * @file src/routes/api/http/auth/saml/acs/+server.ts
...
export const POST = apiHandler(async (event) => {
  ...
  if (!email || !samlId) {
    throw new AppError(
      "SAML response missing critical profile data (email or ID)",
      400,
      "SAML_INVALID_PROFILE",
    );
  }

  if (isMultiTenant && tenantId === "default") {
    throw new AppError(
      "SAML response missing tenant context in multi-tenant mode",
      400,
      "SAML_TENANT_MISSING",
    );
  }
  ...
  if (!user) {
    ...
    if (user && user.samlId && user.samlId !== samlId) {
      ...
      throw new AppError(
        "Account linked to another identity provider",
        403,
        "SAML_IDENTITY_COLLISION",
      );
    }
  }
  ...
  cookies.set(sessionCookie.name, sessionCookie.value, {
    ...(sessionCookie.attributes as Record<string, any>),
    httpOnly: true,
