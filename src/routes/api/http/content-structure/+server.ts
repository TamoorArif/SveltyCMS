/**
 * @file src/routes/api/http/content-structure/+server.ts
 * @description Unified API endpoint for managing content structure metadata
 *
 * @example GET /api/http/content-structure?operation=getContentStructure
 *
 * #Features:
...
export const GET = apiHandler(async ({ url, locals }) => {
  ...
  try {
    ...
    logger.error("Error in GET /api/http/content-structure:", message);
    if (err instanceof AppError) {
      throw err;
    }
    throw new AppError(
      `Failed to process content structure request: ${message}`,
      500,
      "CONTENT_STRUCTURE_ERROR",
    );
  }
});
...
export const POST = apiHandler(async ({ request, locals }) => {
  ...
  try {
    ...
    switch (action) {
      case "reorderContentStructure": {
        ...
        if (!(items && Array.isArray(items))) {
          throw new AppError(
            "Items array is required for reorderContentStructure",
            400,
            "INVALID_ITEMS",
          );
        }
      ...
      case "updateContentStructure": {
        ...
        if (!(items && Array.isArray(items))) {
          throw new AppError(
            "Items array is required for updateContentStructure",
            400,
            "INVALID_ITEMS",
          );
        }
    ...
    const message = err instanceof Error ? err.message : String(err);
    logger.error("Error in POST /api/http/content-structure:", message);
    if (err instanceof AppError) {
      throw err;
    }
    throw new AppError(
...
export const PUT = apiHandler(async ({ request, locals }) => {
  ...
  try {
    ...
    logger.error("Error in PUT /api/http/content-structure:", errorMessage);
    if (err instanceof AppError) {
      throw err;
    }
    throw new AppError(
      `Failed to update content structure: ${errorMessage}`,
      500,
      "CONTENT_STRUCTURE_ERROR",
    );
  }
