...
export class ContentModule {
	...
	public readonly drafts = {
		create: (draft: EntityCreate<ContentDraft>): Promise<DatabaseResult<ContentDraft>> => this.adapter.crud.insert('content_drafts', draft as any),
		...
		getForContent: (contentId: DatabaseId, options?: PaginationOptions): Promise<DatabaseResult<PaginatedResult<ContentDraft>>> =>
			this.adapter.wrap(async () => {
				const res = await this.adapter.crud.findMany<ContentDraft>('content_drafts', { contentId } as any, options as any);
				return this.adapter.utils.createPagination(res.success ? res.data || [] : [], options || {});
			}, 'GET_DRAFTS_FAILED'),

		restore: async (draftId: DatabaseId): Promise<DatabaseResult<void>> => this.adapter.crud.restore('content_drafts', draftId),

		delete: (draftId: DatabaseId): Promise<DatabaseResult<void>> => this.adapter.crud.delete('content_drafts', draftId),
	...
	public readonly nodes = {
		getStructure: (
			...
			this.adapter.crud.findMany('system_content_structure', (options?.filter || {}) as any, { tenantId: options?.tenantId || undefined }),

		upsertContentStructureNode: (node: EntityCreate<ContentNode>): Promise<DatabaseResult<ContentNode>> =>
		...
		update: (path: string, changes: Partial<ContentNode>): Promise<DatabaseResult<ContentNode>> =>
			this.adapter.wrap(async () => {
				const res = await this.adapter.crud.updateMany('system_content_structure', { path } as any, changes as any);
				if (!res.success || res.data?.modifiedCount === 0) throw new Error('Node not found');
				const updated = await this.adapter.crud.findOne<ContentNode>('system_content_structure', { path } as any);
		...
		deleteMany: (paths: string[], options?: { tenantId?: string | null }): Promise<DatabaseResult<{ deletedCount: number }>> =>
			this.adapter.crud.deleteMany('system_content_structure', { path: { $in: paths } } as any, { tenantId: options?.tenantId }),
	...
	public readonly revisions = {
		...
		getHistory: (contentId: DatabaseId, options?: PaginationOptions): Promise<DatabaseResult<PaginatedResult<ContentRevision>>> =>
			this.adapter.wrap(async () => {
				const res = await this.adapter.crud.findMany<ContentRevision>('content_revisions', { contentId } as any, options as any);
				return this.adapter.utils.createPagination(res.success ? res.data || [] : [], options || {});
			}, 'GET_REVISIONS_FAILED'),

		restore: async (revisionId: DatabaseId): Promise<DatabaseResult<void>> => this.adapter.crud.restore('content_revisions', revisionId),

		delete: (revisionId: DatabaseId): Promise<DatabaseResult<void>> => this.adapter.crud.delete('content_revisions', revisionId),
		...
		cleanup: (contentId: DatabaseId, keepLatest: number): Promise<DatabaseResult<{ deletedCount: number }>> =>
			this.adapter.wrap(async () => {
				const history = await this.revisions.getHistory(contentId, { pageSize: 1000, sortField: 'version', sortDirection: 'desc' });
				if (!history.success || (history.data?.items.length || 0) <= keepLatest) return { deletedCount: 0 };
