  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (insertResult.success) {
        ...
        console.warn(
          "MongoDB Write Verification Failed:",
          insertResult.message || insertResult.error,
        );
        console.warn(
          "Skipping functional tests as database requires authentication but credentials might be missing or invalid.",
        );
        console.warn(
          "To run these tests locally, ensure DB_USER and DB_PASSWORD are set in config/private.test.ts",
        );
        db = null;
      }
    } catch (err: any) {
      ...
      console.warn(
        "Skipping remaining tests due to DB connection failure. Ensure DB_USER and DB_PASSWORD are set in config/private.test.ts",
      );
