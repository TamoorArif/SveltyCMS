...
jobSchema.statics = {
	async getNextReady(limit = 10, tenantId: string | null = null): Promise<DatabaseResult<Job[]>> {
		try {
			...
			return { success: false, message: 'Failed to fetch ready jobs', error: { code: 'JOB_FETCH_ERROR', message: err.message } };
		}
	},
	...
	async list(options?: PaginationOption & { status?: string; taskType?: string }): Promise<DatabaseResult<Job[]>> {
		try {
			...
			const err = error as Error;
			return { success: false, message: 'Failed to list jobs', error: { code: 'JOB_LIST_ERROR', message: err.message } };
		}
	},
	...
	async count(filter?: Record<string, unknown>): Promise<DatabaseResult<number>> {
		try {
			...
			const err = error as Error;
			return { success: false, message: 'Failed to count jobs', error: { code: 'JOB_COUNT_ERROR', message: err.message } };
		}
	}
};
...
export const JobModel = (mongoose.models?.Job as JobModelType | undefined) || mongoose.model<Job, JobModelType>('Job', jobSchema);
