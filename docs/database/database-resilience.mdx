...
- Self-healing reconnection lifecycle for MongoDB and SQL (Drizzle)
- Centralized health checks and metrics
- Connection pool diagnostics (API + dashboard widget)
- SQL Migration safety (Drizzle `db:push` / `runMigrations`)
...
- `src/databases/cache-service.ts` (Redis connection)
- `src/databases/mongodb/mongodb-adapter.ts` (reconnection handlers)

| Feature                                           | Status         | Key Artifacts                                                                     |
| ------------------------------------------------- | -------------- | --------------------------------------------------------------------------------- |
| Automatic retry with exponential backoff & jitter | ✅ Implemented | `DatabaseResilience.executeWithRetry()`; used in `db.ts`, `cache-service.ts`      |
| Promise-Based Locking (`_initPromises`)           | ✅ Implemented | `MongoDBAdapter`, `SQLiteAdapter` - Prevents race conditions during startup       |
| Capability-Based Guards                           | ✅ Implemented | `SchedulerService` - Declares explicit dependencies via `db.ensure<Feature>()`    |
| Admin email notification on persistent failure    | ✅ Implemented | `database-failure.svelte`; notification logic in `DatabaseResilience.ts`          |
| Connection pool diagnostics                       | ✅ Implemented | API: `/api/database/pool-diagnostics`; Widget: `database-pool-diagnostics.svelte` |
| Self-healing Reconnection                         | ✅ Implemented | MongoDB/SQL event handlers + `attemptReconnection()`                              |
| Circuit Breaker Pattern                           | ✅ Implemented | `DatabaseResilience.executeWithRetry()`; states: CLOSED, OPEN, HALF_OPEN          |
| Automatic Driver Installation & Fallback          | ✅ Implemented | `install-driver/+server.ts`; manual command on failure                            |
| SQL Migration Strategy                            | ✅ Implemented | `runMigrations(db)` for Drizzle adapters                                          |
...
Pool diagnostics are available via `GET /api/database/pool-diagnostics` and surfaced in `database-pool-diagnostics.svelte`.
...
The resilience system now includes a robust **Circuit Breaker** to prevent cascading failures during database outages.

- **States**:
  - `CLOSED`: Normal operation. Requests flow through.
  - `OPEN`: Failures exceeded threshold. Requests fail-fast immediately without hitting the DB.
  - `HALF_OPEN`: Cooldown elapsed. Testing one request to see if the service recovered.
- **Recovery Policy**:
  - After **30 seconds** in `OPEN` state, the breaker automatically moves to `HALF_OPEN`.
  - It allows **one** probe request: if successful, it moves to `CLOSED`; if it fails, it returns to `OPEN` for another 30s.
- **Config**:
  - `failureThreshold`: 5 consecutive errors.
  - `cooldownMs`: 60,000 (1 minute) cooldown.
- **Monitoring**: States are tracked in `SystemState` and surfaced on the dashboard.

Applied in:

- Virtual folders initialization (`src/databases/db.ts`)
- Database adapter loading (`src/databases/db.ts`)
- Redis connection (`src/databases/cache-service.ts`)

### Self-Healing Database Reconnection

The system handles reconnection identically across adapters using the established lifecycle events.

- **MongoDB**: Mongoose connection handlers (`disconnected`, `error`, `reconnected`)
- **SQL (Drizzle)**: Adapter-level error detection and `attemptReconnection()` in `executeWithRetry()` loops.
...
- Widget: `database-pool-diagnostics.svelte`
...
- Template: `src/components/emails/database-failure.svelte`
...
const successRate = status.metrics.totalRetries
  ? (status.metrics.successfulRetries / status.metrics.totalRetries) * 100
  : 100;
```
