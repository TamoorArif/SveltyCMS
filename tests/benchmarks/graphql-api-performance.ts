...
async function measureGraphQL(name: string, query: string, variables: any = {}, headers: Record<string, string> = {}): Promise<BenchResult> {
	console.log(`\n🧪 Benchmarking: ${name}...`);
...
async function runGraphQLBenchmark() {
	...
	try {
		...
		`;
		results.push(await measureGraphQL('Introspection (Basic)', introspectionQuery, {}, authHeaders));
