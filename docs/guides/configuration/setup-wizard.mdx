...
- [Solution Presets](#solution-presets)
...
<Callout type="success">
  **Zero-Restart Architecture**: Unlike traditional CMSs that require server restart after initial
  setup, SveltyCMS uses dynamic in-memory configuration loading (`initializeWithConfig()`). The
  system transitions from `IDLE` → `READY` state instantly, with cache warming ensuring the first
  page load is blazing fast.
</Callout>
...
```text
src/routes/setup/
├── +page.svelte               # Main orchestrator UI
├── +page.server.ts            # Server-side actions and load function
├── setup-service.server.ts    # Tree-shakeable DB driver and setup logic
├── error-classifier.ts        # Consolidated DB error mapping and classification
├── database-config.svelte     # Step 1: Database setup
├── admin-config.svelte        # Step 2: Admin user
├── system-config.svelte       # Step 3: System settings
├── email-config.svelte        # Step 4: SMTP (optional)
├── review-config.svelte       # Step 5: Review & complete
└── welcome-modal.svelte       # First-time welcome modal
```
...
1. `POST /setup?/seedDatabase` (if not already called)
2. `POST /setup?/completeSetup`
...
sequenceDiagram
    ...
    participant Svr as SvelteKit Server
    participant FS as File System (private.ts)
    participant DB as Database
    participant Sys as System State

    Note over U, Sys: Transitioning from IDLE to READY

    U->>W: Click "Complete Setup"
    W->>W: Final Validation
    W->>Svr: POST /setup?/completeSetup

    Svr->>FS: Write private.ts (Keys, DB Config)
    Svr->>FS: Read back real JWT_SECRET & ENCRYPTION_KEY

    Svr->>DB: Create Admin User & Default Roles
    Svr->>DB: Seed System Settings (per-tenant)

    Svr->>Sys: initializeWithConfig(realKeys)
    Sys->>Sys: Register Adapters
    Sys->>Sys: Warm Content Cache

    Svr->>Svr: Create Session Cookie (Signed with real JWT_SECRET)

    Svr-->>W: Success Response
    W->>W: Clear localStorage
    W-->>U: Redirect to Dashboard

    Note over U, Sys: System is now READY (No restart occurred)
```
...
- [Setup Actions Reference](/docs/api/setup-actions-api.mdx) - Complete API documentation
