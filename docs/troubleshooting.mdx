...
1. **Use the Manual Command:**
   ...
   - Run the command shown (e.g., `npm install mongoose`).
   - Refresh the page to retry the check.
...
db.widgets.insertOne({
	...
	type: "custom",
	category: "display",
	path: "src/widgets/display/custom-widget.svelte",
	enabled: true,
	createdAt: new Date()
})
```
...
2. **Check widget export:**
   ...
   ```javascript
   // ✅ Correct
   export default my-widget;

   // ❌ Wrong
   export { my-widget }; // Named export not supported
   ```
...
```
Failed to load widget: Cannot find module './widgets/custom-widget.svelte'
Error: Widget 'custom-widget' not found
```
...
1. **Check file path and naming:**
   ...
   ls src/widgets/display/custom-widget.svelte
   ```
...
2. **Verify import in widget factory:**
   ...
   const displayWidgets = {
     "custom-widget": () => import("./display/custom-widget.svelte"),
   };
   ```
...
3. **Check widget is enabled:**
   ...
   ```javascript
   // Database check
   db.widgets.findOne({ name: "custom-widget" });
   // Verify: { enabled: true }
   ```
