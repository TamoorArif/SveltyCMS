...
```http
POST /api/system-virtual-folder
```
...
```http
GET /api/system-virtual-folder/[id]
```
...
```http
PATCH /api/system-virtual-folder/[id]
```
...
```http
DELETE /api/system-virtual-folder/[id]
```
...
```http
PATCH /api/system-virtual-folder
```
...
await fetch("/api/system-virtual-folder", {
  method: "POST",
  credentials: "include",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
const response = await fetch("/api/system-virtual-folder/root", {
  credentials: "include",
});
...
// Update folder
await fetch("/api/system-virtual-folder/folder123", {
  method: "PATCH",
  credentials: "include",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
...
- [Setup Actions API](/docs/api/setup-actions-api.mdx)
...
- `src/routes/api/send-mail/+server.ts` - Email sending
- `src/routes/api/system-virtual-folder/+server.ts` - Virtual folder management
- `src/routes/api/system-virtual-folder/[id]/+server.ts` - Individual folder operations
