...
6. **Enterprise SSO**: SAML 2.0 integration via BoxyHQ Jackson (Okta, Azure AD) with JIT provisioning
7. **2FA Security**: TOTP-based two-factor authentication
8. **Cache-Optimized**: Leverages CacheService for high-performance lookups
...
graph TD
    subgraph Request["Request Layer"]
        A[HTTP Request with credentials/token/session/SAML]
    end

    subgraph Auth["Layer 1: Authentication"]
        B["AuthService (login, OAuth, SAML, 2FA)"]
        B1["Files: index.ts, googleauth.ts, saml-auth.ts, two-factor-auth.ts"]
    end

    subgraph Session["Layer 2: Session Management"]
        C["SessionManager"]
        C1["Files: sessionManager.ts, sessionCleanup.ts"]
    end

    subgraph Authorize["Layer 3: Authorization"]
        D["Permissions / RBAC"]
        D1["Files: permissions.ts, corePermissions.ts, apiPermissions.ts"]
    end

    subgraph Resource["Protected Resource"]
        E[API Endpoints / Pages]
    end
...
4. **OAuth & Enterprise SSO**: Google OAuth flow and SAML 2.0 IdP federation
...
- **2FA**: Coordinated via `two-factor-auth.ts`
- **OAuth**: Google OAuth via `googleauth.ts`
- **Enterprise SSO**: SAML 2.0 via `saml-auth.ts` (BoxyHQ Jackson)
...
## File 10: `auth/two-factor-auth.ts` (336 lines)
...
## File 12: `auth/two-factor-authTypes.ts` (30 lines)
...
1. User Login Request
   ...
   │
2. If 2FA Enabled
   ├─> auth/two-factor-auth.ts: verifyTwoFactorCode()
...
All of the following security measures are **fully implemented** in SveltyCMS:
...
8. ✅ **Enterprise SSO (SAML 2.0)**: Integrated via BoxyHQ Jackson with JIT provisioning, supporting Okta, Azure AD, and Auth0
9. ✅ **Permission Caching**: 1-minute TTL to ensure fresh permissions
10. ✅ **Audit Logging**: All auth operations logged via crypto-chained (SHA-256) tamper-evident audit logs
11. ✅ **UUIDv4 Identifiers**: All resource IDs use UUIDv4, preventing sequential enumeration attacks
12. ✅ **Safe Query Mapping**: Database-agnostic query builder prevents injection attacks
13. ✅ **ASR Threat Detection**: Enterprise-grade real-time WAF-like analysis for SQLi, XSS, and command injection
14. ✅ **Plugin Sandbox**: Strict security boundaries for plugins with scoped DB access and resource limits
...
- **Comprehensive Security**: Multi-layer authentication with 2FA and SAML SSO support
...
- **Session Management**: Secure session storage with automatic cleanup and rotation
- **OAuth & Enterprise SSO**: Google OAuth + SAML 2.0 (Okta, Azure AD, Auth0) with JIT provisioning
- **High Performance**: Leverages cache for fast lookups (98% hit rate)
- **API Protection**: Endpoint permission mapping for secure APIs
- **Audit Ready**: Crypto-chained (SHA-256) tamper-evident audit logs for compliance

**Total Authentication System**: ~3,000 lines across 13 files (including `saml-auth.ts`), providing enterprise-grade security for SveltyCMS.
