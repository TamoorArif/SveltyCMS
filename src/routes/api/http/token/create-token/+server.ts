/**
 * @file: src/routes/api/http/token/create-token/+server.ts
...
export const POST = apiHandler(async ({ request, locals, fetch, url }) => {
  try {
    ...
    const [existingUser, existingTokens] = await Promise.all([
      auth.checkUser(checkCriteria),
      auth.getAllTokens(checkCriteria),
    ]);

    if (existingUser) {
      logger.warn("Attempted to create token for an already existing user in this tenant", {
        email: validatedData.email,
        tenantId,
      });
      throw new AppError(
    ...
    const emailResponse = await fetch(`${url.origin}/api/http/send-mail`, {
      method: "POST",
      headers: {
    ...
    if (!emailResponse.ok) {
      ...
      logger.error(
        "Failed to send invitation email. Keeping token for manual/alternate delivery.",
        {
          email: validatedData.email,
          error: emailError,
        },
      ...
      return json({
        ...
        message:
          emailError.message ||
          "Invitation email could not be sent (dev mode). Token preserved for manual delivery.",
    ...
    if (err instanceof Error && err.name === "ValiError") {
      const valiError = err as unknown as {
