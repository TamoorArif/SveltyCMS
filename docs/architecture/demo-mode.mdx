...
1.  **Creation**:
    - **Method A (No Token)**: User visits `/login` -> Click "Sign Up".
      - If `MULTI_TENANT=true`, they can register _without_ a token.
    ...
    - **Method B (Automatic)**: `handleAuthentication` hook detects missing `demo_tenant_id` cookie.
      ...
      - Sets `demo_tenant_id` cookie with a 20-minute expiration.
...
3.  **Expiration & Cleanup**:
    ...
    - It identifies tenanted users created past the cleanup TTL (default: 60 minutes) or those with expired sessions.
