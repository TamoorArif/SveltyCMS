...
- **Middleware Gating**: All requests pass through `handleSystemState` and `handleAuthorization`.
...
- **REST CRUD**: All routes (`/api/crud/[collection]`) append a mandatory `tenantId` filter to every query.
- **GraphQL Yoga**: The schema is filtered based on the requester's tenant permissions, and resolvers utilize the **Vite+** context to ensure data isolation.
...
- **AST Depth Limitation**: Limits query depth (default 7) to prevent nested relation Denial of Service (DoS) attacks.
