	...
	if (locals.user) {
		...
		if (!isApi && !isConfig && !isUser && !isDashboard && !isLogin) {
			...
			if (collections.length === 0) {
				...
				if (locals.isAdmin) {
					logger.info(`[handleContentInitialization] No collections found for tenant: ${tenantId}. Redirecting Admin to builder.`);
					throw redirect(302, '/config/collectionbuilder');
				}
				if (pathname !== '/dashboard') {
					logger.info(`[handleContentInitialization] No collections found for tenant: ${tenantId}. Redirecting to dashboard.`);
