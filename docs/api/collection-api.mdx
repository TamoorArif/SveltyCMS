...
Field definitions support granular uniqueness control in multi-tenant environments:

- **`unique`**: (boolean) Enables the uniqueness constraint for the field.
- **`tenantScopedUnique`**: (boolean, default: `true`) If `true`, uniqueness is only enforced within the current tenant. This allows the same value (e.g., a slug like `/about`) to exist across different tenants.
- **`disableUnique`**: (boolean, default: `false`) Forcefully disables the uniqueness check, even if `unique` is set. Useful for emergency overrides or specific migration scenarios.
...
```typescript
{
  ...
  db_fieldName: "slug",
  unique: true,
  tenantScopedUnique: true // Different tenants can share the same slug
}
```
...
| Data Export          | 1      | Test GET /api/export-data endpoint                |
...
it("should get collection entries with pagination", async () => {
  const response = await fetch(
    `${API_BASE_URL}/api/collections/${collectionId}?page=1&pageSize=10`,
    {
      headers: { Authorization: `Bearer ${authToken}` },
    },
  );
...
```typescript
import { getData, postData, patchData, deleteData } from "@utils/api-client";
