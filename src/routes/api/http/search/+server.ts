/**
 * @file src/routes/api/http/search/+server.ts
 * @description API endpoint for searching across collections
 *
 * @example: GET /api/http/search?q=searchterm&collections=posts,pages&limit=10
 *
 * Features:
...
export const GET = apiHandler(async ({ locals, url }) => {
  ...
  try {
          ...
          if (items.length > 0) {
            try {
              await modifyRequest({
                ...
                fields:
                  collection.fields as unknown as import("@src/content/types").FieldInstance[],
              ...
              const errMsg =
                modifyError instanceof Error ? modifyError.message : String(modifyError);
    ...
    logger.info(
      `Search completed: ${paginatedResults.length}/${totalResults} results in ${duration.toFixed(2)}ms`,
      { tenantId },
    );
