/**
 * @file src/routes/api/http/collections/[collectionId]/+server.ts
 * @description API endpoint for creating new collection entries
 *
 * @example POST /api/http/collections/:collectionId
 *
 * Features:
...
export const POST = apiHandler(async ({ locals, params, request }) => {
  ...
  const endpoint = `POST /api/http/collections/${params.collectionId}`;
  ...
  const sourceData = (body.data && typeof body.data === "object" ? body.data : body) as Record<
    string,
    unknown
  >;
  ...
  if (!dbAdapter) {
    throw new AppError(
      "Service Unavailable: Database service is not properly initialized",
      503,
      "DB_ADAPTER_MISSING",
    );
  }
  ...
  await contentManager.invalidateSpecificCaches(
    [schema.path || "", schema._id as string].filter(Boolean),
  );
