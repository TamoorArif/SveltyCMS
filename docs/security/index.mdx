...
SveltyCMS is designed with a **zero-backdoor philosophy** to ensure maximum security. This means:

### 🔒 No Test Backdoors

- **No API Keys/Bearer Tokens for Testing**: Integration tests cannot bypass authentication via API keys or tokens
- **No Test-Only Bypass Headers**: There are no special headers (like `x-test-token`) that skip authentication
- **No Hardcoded Test Credentials**: Production and test environments use completely separate configurations

### 🧪 Secure Testing Approach

Our integration tests authenticate like real users:

1. **Setup Phase**: Tests use the `/api/testing` endpoint (only enabled in `TEST_MODE`) to:
...
3. **Test Phase**: Authenticated requests proceed normally through the security middleware

### ✅ Why This Matters

- **Prevents Accidental Backdoors**: No code paths that could be exploited
- **Tests Real Security**: Authentication and authorization are tested end-to-end
- **Compliance Ready**: Audit logs show real user-like behavior
- **No Secrets in Code**: Test credentials are generated at runtime, never committed
...
| Issue                            | File                             | Fix Date   |
| -------------------------------- | -------------------------------- | ---------- |
| Rate limiting bypass (localhost) | `src/hooks/handle-rate-limit.ts` | March 2026 |
| Firewall bypass (TEST_MODE)      | `src/hooks/handle-firewall.ts`   | March 2026 |
...
- [x] All widgets reviewed for security - `docs/security/widget-security.mdx`
- [x] Custom widgets follow security guidelines - `docs/security/widget-security.mdx`
...
- [x] Output encoding verified - `docs/security/richtext-security.mdx`
