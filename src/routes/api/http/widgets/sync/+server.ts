/**
 * @file src/routes/api/http/widgets/sync/+server.ts
...
export const POST = apiHandler(async ({ locals, request }) => {
  ...
  try {
    ...
    const dbWidgets: Record<string, unknown>[] = dbResult.success
      ? (dbResult.data as unknown as Record<string, unknown>[]) || []
      : [];
      ...
      try {
        ...
        if (exists) {
          ...
          if (isCore && dbWidget && !(dbWidget.isActive as boolean)) {
            await locals.dbAdapter.system.widgets.update(
              dbWidget._id as unknown as import("@databases/db-interface").DatabaseId,
              { isActive: true },
            );
