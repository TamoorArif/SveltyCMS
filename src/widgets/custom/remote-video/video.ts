 ...
 */
import { getPrivateSettingSync } from "@src/services/settings-service"; // Assuming this is server-side access to private env vars
...
async function fetchTikTokMetadata(url: string): Promise<ExternalVideoMetadata | null> {
  logger.warn(
    "TikTok metadata fetching is experimental and relies on web scraping which can be unreliable.",
  );
  try {
