...
1.  **God-Mode API (`/api/testing`)**: Allows tests to programmatically reset the database and seed specific fixtures without direct DB access.
    - **New Actions**: `get-user` and `get-user-count` enable precise state verification without internal imports.
2.  **Mock Isolation**: Integration tests use real databases (isolated per run) while Unit tests use `tests/unit/setup.ts` for deep internal mocking.
...
- **Files**: `tests/integration/api/auth-2fa.test.ts`, `security.test.ts`, `signup.test.ts`
...
- **100% Coverage**: Targeting the remaining 12 utility endpoints.
