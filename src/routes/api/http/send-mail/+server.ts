/**
 * @file src/routes/api/http/send-mail/+server.ts
...
export const POST = apiHandler(async ({ request, locals }) => {
  ...
  if (!(isAuthorizedInternal || isAdmin)) {
    logger.warn("Unauthorized attempt to access /api/http/send-mail", {
      userId: user?._id,
      tenantId,
  ...
  if (isAuthorizedInternal) {
    logger.debug("Authorized internal API call to /api/http/send-mail", {
      tenantId,
    });
  } else {
    logger.debug(`Admin '${user?.email}' calling /api/http/send-mail`, { tenantId });
  }
