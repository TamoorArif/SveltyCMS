/**
 * @file src/routes/api/http/user/delete-avatar/+server.ts
 ...
 *
 * Usage:
 * DELETE /api/http/user/delete-avatar
...
export const DELETE: RequestHandler = apiHandler(async ({ request, locals }) => {
  ...
  if (getPrivateSettingSync("MULTI_TENANT") && !isEditingSelf) {
    ...
    if (!targetUser || targetUser.tenantId !== tenantId) {
      ...
      throw new AppError(
        "Forbidden: You can only delete avatars for users within your own tenant.",
        403,
        "FORBIDDEN_TENANT",
      );
    }
  ...
  if (avatarToDelete && avatarToDelete !== "/Default_User.svg") {
    try {
      ...
      logger.info("Avatar moved to trash successfully", {
        userId: user._id,
      ...
      if (moveError instanceof Error && moveError.message.includes("ENOENT")) {
        ...
        logger.error(`Failed to move avatar file to trash: ${errorMsg}`, {
          userId: user._id,
        });
    ...
    logger.info("No avatar to move to trash for user.", {
      userId: user._id,
      tenantId,
    });
  ...
  logger.info("User avatar attribute removed from profile.", {
    userId: targetUserId,
  ...
  try {
    ...
    await cacheService.clearByPattern("api:*:/api/http/user*", tenantId);
    ...
    logger.warn("Failed to invalidate cache after avatar deletion", {
      error: cacheError,
    });
  }
