/**
 * @file src/routes/api/http/collections/[collectionId]/[entryId]/revisions/+server.ts
 * @description API endpoint for retrieving entry revision history
 *
 * @example: GET /api/http/collections/posts/123/revisions
 *
 * Features:
...
export const GET = apiHandler(async ({ locals, params, url }) => {
  ...
  return json({
    ...
    data: {
      ...
      totalPages: paginatedResult.total
        ? Math.ceil(paginatedResult.total / paginatedResult.pageSize)
        : undefined,
    },
