...
- **Direct Edit**: A dedicated **"Edit" icon** on image thumbnails allows for instant navigation to the Image Editor, pre-loading the asset for manipulation.
...
- **Adaptive Transcoding Hub**: Multi-resolution pipeline for video assets.
  - **HLS Adaptive Bitrate**: Automatically generates `.m3u8` master playlists and chunked TS files.
  - **Multi-Res MP4**: Generates 1080p, 720p, 480p, and 360p versions for fallback compatibility.
  - **Background Processing**: Offloads heavy `ffmpeg` tasks to ensure UI responsiveness.
- **Batch Image Processor**: Professional-grade bulk manipulation.
  - **Sharp.js Filters**: Apply Vivid, B&W, Sepia, and HDR filters to 100+ images simultaneously.
  - **Bulk Transformation**: Resize, rotate, and convert formats in one operation.
  - **Non-Destructive Variants**: Generates linked variants while preserving original uncompressed assets.
...
### 🖼️ Document Thumbnails (ImageMagick)
...
The Media Gallery supports a high-efficiency workflow through specialized keyboard shortcuts and WCAG 3.0 focus management.

| Shortcut  | Action                                                                  |
| :-------- | :---------------------------------------------------------------------- |
| `Mod + F` | **Focus Search**: Instantly jump to the global search input.            |
| `Mod + A` | **Select All**: Select every asset in the current filtered view.        |
| `Delete`  | **Bulk Removal**: Trigger a confirmation to delete all selected assets. |
| `Escape`  | **Clear**: Deselect all items or clear the search query.                |
| `Tab`     | **Grid Navigation**: Move focus between asset thumbnails sequentially.  |
...
All media operations are exposed via a type-safe REST API. For a complete reference, see the [Media API Documentation](../api/media-api.mdx).
...
- [**Media API Reference**](../api/media-api.mdx) - Detailed endpoint documentation.
