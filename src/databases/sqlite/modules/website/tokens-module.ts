  ...
  async create(
    ...
    return this.core.wrap(async () => {
      ...
      await this.db
        ...
        .values(insertData as typeof schema.websiteTokens.$inferInsert);
      const [result] = await this.db
        .select()
        .from(schema.websiteTokens)
        .where(eq(schema.websiteTokens._id, id))
        .limit(1);
  ...
  async getAll(options: {
    ...
    return this.core.wrap(async () => {
      ...
      if (options.sort) {
        ...
        const column = (
          schema.websiteTokens as unknown as Record<
            string,
            import("drizzle-orm/sqlite-core").SQLiteColumn
          >
        )[options.sort];
        if (column) {
