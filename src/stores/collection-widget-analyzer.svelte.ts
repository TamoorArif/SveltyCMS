 ...
 */
export function analyzeCollectionWidgets(
  schema: Schema,
  activeWidgets: string[],
): CollectionWidgetDependency {
 ...
 */
export function analyzeMultipleCollections(
  schemas: Schema[],
  activeWidgets: string[],
): CollectionWidgetDependency[] {
...
export function canSafelyDisableWidget(
  ...
  const affectedCollections = analyses
    .filter((analysis) => analysis.requiredWidgets.includes(widgetName))
    .map((analysis) => analysis.collectionName);
...
export function validateCollections(
  ...
  for (const analysis of invalid) {
    warnings.push(
      `Collection "${analysis.collectionName}" is missing widgets: ${analysis.missingWidgets.join(", ")}`,
    );
  }
