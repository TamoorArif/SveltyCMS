...
```
Frontend/API → db.ts → dbInterface → Adapter (MongoDB/SQL) → Modules/Methods → Database
```

**Method Modules Location:**

- **MongoDB**: `src/databases/mongodb/methods/`
- **SQL (Drizzle)**: `src/databases/sqlite/` (and variants, e.g., `sqlite/crud/`, `sqlite/modules/auth/`)

---

## 🚀 Database Initialization (db.ts)

Before diving into method modules, it's essential to understand the **database initialization layer** provided by `src/databases/db.ts`.

### Initialization Functions

```typescript
// src/databases/db.ts - Core initialization functions

/**
 * Standard initialization - loads config from filesystem
 * Used during: Normal server startup
 */
async function initializeSystem(forceReload = false, skipSetupCheck = false): Promise<void>;

/**
 * Zero-restart initialization - accepts config in-memory
 * Used during: First-time setup completion (bypasses Vite cache)
 */
export async function initializeWithConfig(config: PrivateEnv): Promise<{ status: string }>;

/**
 * Re-initialization - forces system reload
 * Used during: Configuration updates, adapter swaps
 */
...
### initializeWithConfig() - Zero-Restart Setup

**Problem:** When users complete the setup wizard, `config/private.ts` is created, but Vite caches module imports. Traditional approach required server restart.

**Solution:** Pass configuration in-memory, completely bypassing filesystem imports.
...
sequenceDiagram
    ...
    participant System as initializeSystem()
    participant State as System State

    Setup->>File: Write configuration file
    Note over File: File created on disk

    Setup->>DB: initializeWithConfig(fullConfig)
    Note over DB: Config passed in-memory<br/>(no filesystem import)

    DB->>DB: privateEnv = config
    DB->>System: initializeSystem(false, true)
    Note over System: skipSetupCheck = true<br/>uses in-memory config

    System->>System: Connect to database
    System->>System: Load adapters
    System->>System: Setup models
    State->>State: Set state = READY

    System-->>System: Background task: startBackgroundWarmup()
    Note over State: Transition to WARMING...

    DB-->>Setup: { status: 'success' }

    Note over Setup,State: ✅ System READY for requests<br/>Hydration completes in background
```
...
```typescript
// src/databases/db.ts
let privateEnv: PrivateEnv | null = null; // In-memory config storage

export async function initializeWithConfig(config: PrivateEnv): Promise<{ status: string }> {
  try {
    logger.info("Initializing with provided config (bypassing Vite cache)...");

    // Store config in memory (critical for zero-restart)
    privateEnv = config;
...
async function initializeSystem(forceReload = false, skipSetupCheck = false): Promise<void> {
  ...
  let privateConfig;
  if (skipSetupCheck) {
    // Use in-memory config (from initializeWithConfig)
    logger.debug("Using pre-set configuration (zero-restart mode)");
    privateConfig = privateEnv;
  } else {
    // Normal mode: import from filesystem
    privateConfig = await loadPrivateConfig(forceReload);
  }

  // ... continue with adapter loading, connection, etc.
}
```
...
```typescript
// src/lib/server/setup/complete/+server.ts
export const POST: RequestHandler = async ({ request }) => {
	const setupData = await request.json();

	// 1. Write config file to disk
	await writeConfig(fullConfig);

	// 2. Initialize system with in-memory config (zero-restart!)
	await initializeWithConfig(fullConfig);

	// 3. Create admin user
	await createAdminUser(userData);

	// 4. Warm cache for first page load
	try {
        const { collectionService } = await import('@src/services/CollectionService');
        // ... (fetch collection data internally) ...
        await collectionService.getCollectionData(...);
    } catch (e) {
        // ...
    }

	// User is redirected to /login - system fully operational!
...
stateDiagram-v2
    ...
    IDLE --> INITIALIZING: First request arrives
    IDLE --> INITIALIZING: initializeWithConfig() called

    INITIALIZING --> READY: Critical path healthy
    READY --> WARMING: Background warmup begins
    WARMING --> WARMED: All modules active

    READY --> DEGRADED: Non-critical failure
    INITIALIZING --> FAILED: Critical service failed
    WARMING --> DEGRADED: Background module failed

    READY --> FAILED: Database disconnected
    WARMED --> FAILED: Database disconnected

    FAILED --> [*]: Requires restart

    note right of READY
        - Database connected
        - Auth models registered
...
- **dbInterface (IDBAdapter)** - Abstract interface for database operations
- **Adapter** - Implements IDBAdapter for specific database (MongoDB, PostgreSQL, SQLite, etc.)
- **Modules/Methods** - Modular, reusable logic organized by responsibility (CRUD, Auth, Media, etc.)

**Multilingual Data Abstraction:**
The database adapter handles storage format differences transparently:

```typescript
// Application layer always works with this format:
const entry = {
...
// MongoDB adapter stores nested objects directly:
db.collection("posts").insertOne(entry);
// Result: { title: { en: "Hello", de: "Hallo" }, ... }

// SQL (Drizzle) adapter transforms to relational for better normalization:
// collections table: _id, slug, createdAt, updatedAt
// collection_<id>_translations table:
//   - parent_id, field='title', lang='en', value='Hello'
//   - parent_id, field='title', lang='de', value='Hallo'
//
// Alternatively, SQL adapters may use JSON/JSONB columns (PostgreSQL)
// or serialized TEXT strings (SQLite) while maintaining the same object structure.
//
// On read: Adapter reconstructs { title: { en: "Hello", de: "Hallo" }, ... }
```

**Key Principle:** Widgets and components never know the underlying storage format—they always receive/send the same multilingual object structure.

Each adapter organizes its logic into modules or methods:

| MongoDB Method Module | SQL (Drizzle) Equivalent        | Responsibility                    |
| --------------------- | ------------------------------- | --------------------------------- |
| **authMethods**       | `modules/auth/authModule.ts`    | Authentication & User management  |
| **collectionMethods** | `collection/collectionModule`   | Dynamic schema/model management   |
| **crudMethods**       | `crud/crudModule.ts`            | Generic CRUD operations           |
| **contentMethods**    | `modules/content/contentModule` | CMS-specific content workflows    |
| **mediaMethods**      | `modules/media/mediaModule`     | Media file operations             |
| **themeMethods**      | `modules/themes/themesModule`   | Theme installation & activation   |
| **widgetMethods**     | `modules/widgets/widgetsModule` | Widget registration & management  |
| **systemMethods**     | `modules/system/` modules       | System preferences & settings     |
| **mongoDBUtils**      | `utils.ts`                      | Shared utilities & error handling |
...
Retrieves the persistent schema definition for a collection from the database.
...
Lists all persistent schema definitions stored in the database.
...
export class MongoContentMethods {
  ...
  async getStructure(
    mode: "flat" | "nested",
    filter?: FilterQuery<ContentNode>,
  ): Promise<ContentNode[]>;
  ...
  ): Promise<PaginatedResult<ContentDraft>>;
...
export class MongoSystemMethods {
	constructor(system-preferencesModel: Model<system-preferencesDocument>, systemSettingModel: Model<SystemSetting>);

	// User Preferences
	async setUserPreferences(userId: string, layoutId: string, layout: Layout): Promise<void>;
	async getsystem-preferences(userId: string, layoutId: string): Promise<Layout | null>;
	...
	async clearsystem-preferences(userId: string): Promise<void>;
...
```typescript
const systemMethods = new MongoSystemMethods(system - preferencesModel, SystemSettingModel);
...
(await systemMethods.clearsystem) - preferences("user123");
```
...
export class MongoSystemVirtualFolderMethods implements IDBAdapter["systemVirtualFolder"] {
  ...
  async getContents(
    folderPath: string,
  ): Promise<DatabaseResult<{ folders: SystemVirtualFolder[]; files: any[] }>>;
...
**Purpose:** Shared utilities for database operations (Pure functions, no dependencies on CacheService)
...
### 11. MongoDB Cache Utilities (`mongoDBCacheUtils.ts`)

**Purpose:** Caching utilities for database operations. Separated from `mongoDBUtils` to prevent circular dependencies.
...
```typescript
import { withCache, invalidateCategoryCache } from "./mongoDBCacheUtils";
...
const items = await Promise.all([
  mediaMethods.uploadMany(images),
  widgetMethods.findAllActive(),
  themeMethods.getActive(),
]);
...
For multilingual content, cache keys **must** include the language parameter to prevent serving wrong language data:

```typescript
// ✅ CORRECT: Language-specific cache
const cacheKey = `entries:${collectionId}:page:${page}:lang:${language}`;
const entries = await withCache(
  cacheKey,
  async () => await db.crud.findMany("posts", { collectionId }),
  180,
  "CONTENT",
);

// ❌ WRONG: Missing language - will serve cached EN data when DE is requested
const cacheKey = `entries:${collectionId}:page:${page}`;
```

**Cache Invalidation for Translated Content:**

When updating multilingual content, invalidate cache for **all languages**:

```typescript
async function updateEntry(entryId: string, data: any) {
  // Update database
  await db.crud.update("entries", entryId, data);

  // Clear cache for ALL languages
  const languages = await getAvailableContentLanguages();
  for (const lang of languages) {
    await db.cache.delete(`entry:${entryId}:lang:${lang}`);
  }

  // Or use category invalidation to clear all entry caches
  invalidateCategoryCache("CONTENT");
}
```
...
await mongoose.connection
  .collection("users")
  .updateOne({ _id: userId }, { $set: { name: "New Name" } });
```
...
- [Content Management](/docs/guides/content/smart-importer) - Uses contentMethods
- [Database Resilience](./database-resilience.mdx) - Unified guide for resilience features, integration, metrics, and roadmap
