/**
 * @file src/routes/api/http/collections/[collectionId]/[entryId]/status/+server.ts
 * @description API endpoint for updating entry status
 *
 * @example: PATCH /api/http/collections/posts/123/status
 *
 * Features:
...
export const PATCH = apiHandler(async ({ locals, params, request }) => {
  ...
  if (entries && Array.isArray(entries) && entries.length > 0) {
    ...
    if (!verify.success || verify.data.length !== entries.length) {
      throw new AppError(
        "One or more entries do not belong to your tenant or do not exist.",
        403,
        "FORBIDDEN",
      );
    }

    const result = await dbAdapter.crud.updateMany(
      normalizedName,
      query,
      updateData as Partial<BaseEntity>,
    );
    if (!result.success) {
  ...
  await cacheService
    .clearByPattern(`collection:${schema._id}:*`)
    .catch((e: any) => logger.warn("Cache clear failed", e));
