...
3. **The Mechanics** — *(Optional & Final)* — Explain the architecture, state machines, or DB-agnostic adapters *after* the goal is achieved. Never lead with mechanics for high-level guides.
...
4. **CI parity** *(if code was also changed)*: Run the full check suite.
   ```bash
