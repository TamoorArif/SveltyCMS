---
path: "src/widgets/core/media-upload/mediaupload.mdx"
...
<blockquote>
  **Media management widget** with modal-based library, drag-and-drop upload, multi-file support,
  and optimized file reference storage.
</blockquote>
...
- ✅ **AI-Powered Tagging**: Automatically displays and utilizes AI-generated tags for smarter search and organization. Tags are visible in the media grid and detailed views.
- ✅ **Collection-Specific Storage**: Organize media by collection using the `folder` option (e.g., `folder: 'collection/posts'`). Files are stored in specific subdirectories for better DAM organization.
- ✅ **Real-Time Previews**: Fetches real media data and thumbnails from the API, providing accurate visual feedback in both grid and entry views.
- ✅ **Non-Destructive Variants**: Integrated with the Image Editor to save edited variants while preserving the original.
...
- **Small Icons**: Use `Icon` or `IconPicker` widget
...
export const postsCollection = {
  ...
  fields: [
    Media({
      ...
      folder: "blog/featured", // ✅ Organize by collection/purpose
      multiupload: false,
      required: true,
...
The MediaUpload widget is part of SveltyCMS's **AI-Native DAM Engine**. When a file is uploaded, the following automated processes occur:

1.  **Image Analysis**: Local Ollama models (e.g., `llava`) "see" the image.
2.  **Auto-Tagging**: Descriptive tags are generated and stored in `metadata.aiTags`.
3.  **Visual Feedback**: These tags are displayed as badges in the `Input` and `Display` components, helping editors quickly verify content.
4.  **Smart Search**: AI tags are indexed, enabling semantic search across your entire media library.
...
| `folder`       | `string`   | ❌ No     | `'global'`  | Storage subfolder (e.g. `posts`)       |
...
// Media file document (in 'media_files' collection)
{
  ...
  "path": "collections/posts/original/hero-image-2025-01-15-abc123.jpg",
  "originalId": null, // Linked to original if this is a variant
  "metadata": {
    "aiTags": ["mountain", "sunset", "outdoor"], // ✅ Automated AI tags
