...
export const load: PageServerLoad = async ({ locals, params }) => {
  try {
    ...
    const serializableCollection = currentCollection
      ? deepCloneAndRemoveFunctions(currentCollection)
      : null;
    ...
    for (const key in field) {
      ...
      for (const importKey in widget.GuiSchema) {
        ...
        for (const IMPORT of widgetImport) {
          ...
          const replacement = (typeof importValue === "string" ? importValue : "")
            .replace(/🗑️/g, "")
            .trim();
      ...
      const widgetFnName =
        fieldWithWidget.widget.key ||
        fieldWithWidget.widget.Name ||
        fieldWithWidget.widget.widgetId;
...
export const schema: Schema = {
    ...
    throw new Error(
      `Failed to generate collection file: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
