...
graph TD
    ...
    Runner[Integration Runner <br> 'bun test']:::testBed
...
2.  **Cryptographic Handshake**: Requires a `x-test-secret` header matching a unique UUID generated dynamically for each test run.
...
- **Restricted Role**: Verifies `403 Forbidden` (RBAC enforcement) and redirection to `/forbidden`.
