...
| -------------- | -------------------------- | --------------------------------------------------- |
| **Definition** | ` $args[0].Value.ToLower() | Widget configuration, validation schema, GUI schema |
| **Input**      | ` $args[0].Value.ToLower() | User input component with validation                |
| **Display**    | ` $args[0].Value.ToLower() | Read-only display of stored values                  |
...
| -------------- | ------------------ | ------------------------------------------------ | -------- |
| `label`        | Input              | Display label for the field                      | ✅       |
...
The Input Widget integrates with the built-in AI Service to provide one-click translations. When editing a translatable field in a non-default language, a **Magic Wand** (mdi:translate) icon appears. Clicking this icon:

1. Sends the default language content to the configured AI provider (Ollama/OpenAI).
2. Autonomously generates a high-quality translation for the current target language.
3. Sanitizes and populates the input field instantly.
...
For technical IDs or URLs, use the dedicated [Slug Widget](/src/widgets/core/slug/slug.mdx) which provides specialized normalization and multi-tenancy uniqueness control.
...
export const createValidationSchema = (field) => {
  ...
  if (field.minLength)
    stringRules.push(minLength(field.minLength, `Must be at least ${field.minLength} characters.`));
  if (field.maxLength)
    stringRules.push(
      maxLength(field.maxLength, `Must be no more than ${field.maxLength} characters.`),
    );
...
- [Email Widget](<$args[0].Value.ToLower()>) - For email addresses with format validation
- [Number Widget](<$args[0].Value.ToLower()>) - For numeric values with min/max constraints
- [PhoneNumber Widget](<$args[0].Value.ToLower()>) - For phone numbers with E.164 validation
