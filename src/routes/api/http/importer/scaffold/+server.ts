/**
 * @file src/routes/api/http/importer/scaffold/+server.ts
...
export const POST = apiHandler(async ({ request, locals }) => {
  ...
  if ((isMultiTenant && !isSuperAdmin) || (!isMultiTenant && user.role !== "admin")) {
    ...
    throw new AppError(
      "Forbidden: Only super-admins can scaffold collections in multi-tenant mode.",
      403,
      "FORBIDDEN",
    );
  }

  const { sourceType, sourceUrl, apiKey, sourceTypeIdentifier, collectionName } =
    await request.json();
