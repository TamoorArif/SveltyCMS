...
| ------------------------ | ------- | ----------- | ----------------------------------------------- |
| handleStaticAssetCaching | 6       | ✅ Complete | `tests/unit/hooks/static-asset-caching.test.ts` |
| handleSystemState        | 26      | ✅ Complete | `tests/unit/hooks/system-state.test.ts`         |
| handleRateLimit          | 35      | ✅ Complete | `tests/unit/hooks/rate-limit.test.ts`           |
| handleFirewall           | 27      | ✅ Complete | `tests/unit/hooks/firewall.test.ts`             |
| handleSetup              | 16      | ✅ Complete | `tests/unit/hooks/setup.test.ts`                |
| handleLocale             | 34      | ✅ Complete | `tests/unit/hooks/locale.test.ts`               |
| handleTheme              | 16      | ✅ Complete | `tests/unit/hooks/theme.test.ts`                |
| handleAuthentication     | 33      | ✅ Complete | `tests/unit/hooks/authentication.test.ts`       |
| handleAuthorization      | 23      | ✅ Complete | `tests/unit/hooks/authorization.test.ts`        |
| handleApiRequests        | 28      | ✅ Complete | `tests/unit/hooks/api-requests.test.ts`         |
| handleTokenResolution    | 5       | ✅ Complete | `tests/unit/hooks/token-resolution.test.ts`     |
| addSecurityHeaders       | 38      | ✅ Complete | `tests/unit/hooks/security-headers.test.ts`     |
| **Total**                | **287** | **100%**    | 12 files                                        |

## Performance Benchmarks

In addition to correctness, all hooks are benchmarked for execution overhead using `tests/benchmarks/hooks-performance.ts`.

| Metric                     | Value    | Target (2026) |
| -------------------------- | -------- | ------------- |
| Static Regex Check Latency | 0.125 µs | < 0.200 µs    |
| API Fast-path Skip Latency | 0.048 µs | < 0.100 µs    |
| Compression Guard Latency  | 0.062 µs | < 0.100 µs    |
| Total API Hook Overhead    | 0.150 ms | < 0.500 ms    |
...
it("should track requests by IP+User-Agent", async () => {
  ...
  for (let i = 0; i < 10; i++) {
    await handleRateLimit({
      event: createMockEvent("/api/test", "GET", headers),
      resolve: mockResolve,
    });
  }
...
```bash
bun test tests/unit/hooks/static-asset-caching.test.ts
...
- **Total Hook Tests**: 287
- **Coverage**: 100% (12/12 hooks)
- **Test Files**: 12
- **Average Tests per Hook**: 24
...
| Rate Limiting           | 35    | ✅ Complete |
| Firewall & Security     | 27    | ✅ Complete |
| Setup Wizard            | 16    | ✅ Complete |
| Internationalization    | 34    | ✅ Complete |
...
- [Architecture: Middleware Hooks](../architecture/server-hooks.mdx)
