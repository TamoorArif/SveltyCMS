...
import { GET as getSettings, PUT as updateSettings, DELETE as resetSettings } from '@src/routes/api/settings/[group]/+server';
...
vi.mock('@src/routes/(app)/config/system-settings/settings-groups', () => ({
	getSettingGroup: vi.fn().mockImplementation((id) => ({
		id,
		name: id,
		fields: id === 'site' ? [{ key: 'SITE_NAME', label: 'Site Name', type: 'text', category: 'public' }] : [],
		adminOnly: true
	})),
	settingsGroups: []
}));
...
describe('Settings API Security - Tenant Isolation and RBAC', () => {
	...
	describe('Site-Specific Groups (e.g., site)', () => {
		...
		it('should scope settings to the current tenant for regular admin', async () => {
			...
			await getSettings(event);
			expect(dbAdapter!.system.preferences.getMany).toHaveBeenCalledWith(expect.any(Array), 'system', myTenant);
		});
		...
		it('should allow super-admin to override tenantId', async () => {
			...
			await getSettings(event);
			expect(dbAdapter!.system.preferences.getMany).toHaveBeenCalledWith(expect.any(Array), 'system', otherTenant);
		});
		...
		it('should use tenantId for updates (PUT)', async () => {
			...
			expect(dbAdapter!.system.preferences.setMany).toHaveBeenCalledWith([expect.objectContaining({ userId: myTenant })]);
		});
		...
		it('should use tenantId for resets (DELETE)', async () => {
			...
			expect(dbAdapter!.system.preferences.deleteMany).toHaveBeenCalledWith(expect.any(Array), 'system', myTenant);
		});
	});
});
