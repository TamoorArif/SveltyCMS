...
import { createExportMetadata, encryptSensitiveData, exportCollections, exportSettings } from '@src/utils/server/export-utils';
 ...
 */
async function createExport(userId: string, options: ExportOptions, tenantId?: string): Promise<ExportData> {
	...
	if (options.includeSettings) {
		...
		if (Object.keys(sensitive).length > 0) {
			if (options.includeSensitive && options.sensitivePassword) {
				...
				logger.warn('Sensitive settings requested but no password provided for encryption. Skipping sensitive fields.');
			}
		}
	}
...
export const POST = apiHandler(async ({ request, locals, url }) => {
	...
	if (getPrivateSettingSync('MULTI_TENANT')) {
		...
		if (targetTenantId !== tenantIdFromLocals && !isSuperAdmin) {
			...
			throw new AppError('Unauthorized: You can only export data for your own tenant.', 403, 'TENANT_MISMATCH');
		}
	}
