...
export class PersistentSecurityStore {
	...
	async setThrottle(ip: string, factor: number, untilMs: number): Promise<void> {
		...
		if (ttl > 0) {
			await cacheService.set(`${PREFIX}throttle:${ip}`, JSON.stringify({ factor, until: untilMs }), ttl);
		}
	}

	/** Gets throttle factor for an IP. */
	async getThrottle(ip: string): Promise<{ throttled: boolean; factor: number; until: number } | null> {
