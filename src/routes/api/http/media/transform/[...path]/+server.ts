/**
 * @file src/routes/api/http/media/transform/[...path]/+server.ts
 * @description On-the-fly image transformation API endpoint.
 *
 * @example
 * /api/http/media/transform/path/to/image.jpg?w=800&h=600&fit=cover&q=80&format=webp
 *
 * @features
 * - Resizing (w, h)
...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  try {
    ...
    const width = url.searchParams.get("w")
      ? Number.parseInt(url.searchParams.get("w")!, 10)
      : undefined;
    const height = url.searchParams.get("h")
      ? Number.parseInt(url.searchParams.get("h")!, 10)
      : undefined;
    const quality = url.searchParams.get("q")
      ? Number.parseInt(url.searchParams.get("q")!, 10)
      : 80;
