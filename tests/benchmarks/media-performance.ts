...
async function setup() {
	...
	try {
		...
		await sharp({
			create: { width: 1920, height: 1080, channels: 4, background: { r: 100, g: 150, b: 200, alpha: 1 } }
		})
			.composite([
				{
					input: Buffer.from(
...
function stats(times: number[]): { avg: string; min: string; max: string; p95: string; stddev: string } {
...
async function runBenchmark() {
	...
	console.log(`Node: ${process.version} | CPU cores: ${os.cpus().length} | Thread pool: ${process.env.UV_THREADPOOL_SIZE ?? 4}\n`);
	...
	console.log(` Avg: ${hashStats.avg} ms | min/max/p95: ${hashStats.min}/${hashStats.max}/${hashStats.p95} ms | stddev: ${hashStats.stddev} ms\n`);
	...
	console.log(` Avg: ${metaStats.avg} ms | min/max/p95: ${metaStats.min}/${metaStats.max}/${metaStats.p95} ms | stddev: ${metaStats.stddev} ms\n`);
	...
	for (let i = 0; i < ITERATIONS_HEAVY; i++) {
		...
		for (const width of Object.values(SIZES)) {
			await img.clone().resize(width, null, { fit: 'cover', withoutEnlargement: true }).jpeg({ quality: QUALITY, mozjpeg: true }).toBuffer(); // simulate write
		}
	...
	await Promise.all(
		Array.from({ length: 10 }).map(async () => {
			const img = sharp(originalBuffer);
			for (const width of Object.values(SIZES)) {
				await img.clone().resize(width, null, { fit: 'cover', withoutEnlargement: true }).jpeg({ quality: QUALITY }).toBuffer();
			}
		})
	);
	...
	console.log('-----------------------------------------------------------');
	const syncTotalAvg = parseFloat(hashStats.avg) + parseFloat(metaStats.avg) + parseFloat(resizeStats.avg);
